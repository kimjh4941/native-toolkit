...
| R3-M5 | OP-20 の Application 経路と UseCase が未定義 | **`CancelReceiveFilePromisesUseCase` を追加**。ファイル構成・テスト・T-06 の本数を同期（§6.1 / §12.1 / §13） |
...
- **即時 control / factory に該当する native API（OP-12〜OP-17、OP-19、OP-20）のみ同期**（common.md の control / factory 例外）。`@MainActor` 隔離のため、他アクターからの呼び出し側は `await` する。**OP-01〜OP-11 の native は `async throws`**（§8.1。R3-M7）
...
struct FilePromiseState {
    ...
    var releaseScheduled: Bool = false  // MainActor へ解放を予約済み（R3-H2）
    var generation: UInt64 = 0          // 開始のたびに +1。予約の有効性判定に使う
    var isReleased: Bool = false
    var stagingURL: URL?                // staging の削除責務（R3-H3）
}
```

**解放の 2 段階 claim（R3-H2）**

v3 は「lock 内で `inFlightCount == 0` を判定 → `Task { @MainActor }` で辞書から除去」としていたが、**予約から実行までの間に新しい `writePromiseTo` が開始できる**ため、履行中の provider を除去し得た。v4 は予約を lock 内で一意に claim し、**MainActor 側で同じ lock と generation を使って再判定する**。

```
[lock 内] 解放条件成立 && !releaseScheduled && !isReleased
        -> releaseScheduled = true, g = generation
        -> Task { @MainActor in ... } を 1 回だけ予約

[MainActor / lock 再取得] releaseRequested && inFlightCount == 0
                          && generation == g && !isReleased
        -> isReleased = true, stagingURL を削除, 辞書から除去
   それ以外（新規開始が入った）
        -> releaseScheduled = false（予約を破棄。最後の完了で再予約される）
```
...
| 事象 | lock 内の更新 | 解放予約 |
|---|---|---|
| `writePromiseTo` 開始 | `inFlightCount += 1`、`generation += 1`、**`releaseScheduled = false`（既存予約を無効化）**（**クロージャ実行前に原子的に**） | しない |
| `writePromiseTo` 開始（`isReleased == true`） | 更新しない | **履行せず `completionHandler(filePromiseWriteFailed("promise already released"))` を呼ぶ** |
| `completionHandler` 呼び出し | `inFlightCount -= 1` | 条件成立なら claim |
| `releaseFilePromise(handle:)` | `releaseRequested = true` | 条件成立なら claim |
| stale 検出（`changeCount` 不一致） | `releaseRequested = true` | 条件成立なら claim |
| MainActor 側で再判定 OK | `isReleased = true` | **staging 削除 + 辞書除去（exactly-once）** |
| MainActor 側で再判定 NG | `releaseScheduled = false` | 何もしない（次の完了で再予約） |
...
Bridge 経由の登録では、`sourcePath` の内容をアプリ専用 staging へスナップショットする（§8.4.5）。v3 は staging の作成を Bridge 側、解放を coordinator 側としており、**stale 解放時に staging を消す経路がなかった**。v4 は**作成も削除も coordinator が行う**。

```swift
@MainActor
public protocol ClipboardPromiseRegistry {
    /// staging パスを決めるために handle ID を先に発行する（R3-H3）。
    /// この時点では登録されない。
    func reserveFilePromiseHandle() -> FilePromiseHandle
    /// 予約済み handle に対して staging スナップショットを作り、登録する。
    /// staging コピーに失敗した場合、部分コピーを削除して throw する（登録しない）。
    func registerFilePromise(_ request: FilePromiseRequest,
                             reserved: FilePromiseHandle,
                             snapshotting sourcePath: String?) throws -> FilePromiseHandle
...
- 登録 state の `stagingURL` に保持し、**明示 release / stale 解放 / rollback のすべてが同じ解放経路を通る**ため、削除は exactly-once になる
- `sourcePath` が `nil`（ネイティブ版の `write` クロージャ利用）なら `stagingURL` も `nil`。削除処理は no-op

**transaction 順序（R3-H3 / R2-M12）**

```
1. handle = registry.reserveFilePromiseHandle()          // 登録しない
2. do { try registry.registerFilePromise(req, reserved: handle,
                                         snapshotting: sourcePath) }
   catch { /* staging は registry 内で削除済み。登録もされていない */ throw }
3. do { try repository.writeFilePromise(handle:scope:) }
   catch { registry.releaseFilePromise(handle)   // staging も削除される
           throw }
4. return handle
```
...
|---|---|---|---|---|
| 開始失敗（destination 不正、scope 解決失敗） | **`onEvent` は呼ばれない**。開始 API が throw | **factory 自体が `throws`。stream を作らずに throw**（R3-H1） | throw | 開始 ack callback で `isSuccess=NO` + errorCode |
...
| **Task キャンセル（集約 async）** | 該当なし | 該当なし | **`withTaskCancellationHandler` で OP-20 を呼び、`CancellationError` を throw する**（部分 receipt は破棄。受領済みファイルはディスク上に残る） | 該当なし |

- `filePromiseTimedOut` は削除する。`cancelled` は検出 API（OP-09〜OP-11）と、**集約 async 版の Task キャンセル**でのみ使う
...
`AsyncThrowingStream` の `onTermination` は **stream が終端した後**に呼ばれる。consumer の Task がキャンセルされて終端した stream へ `.finished` を yield しても、consumer には届かない。したがって v3 の「`onTermination` から `.finished` を yield」は成立しない。
...
- **`onTermination` は資源解放専用**。receipt の cancel、静穏 / 全体タイマの停止、staging の削除を行い、イベント配送は試みない
...
- 内部で `CancelReceiveFilePromisesUseCase` を経由して `registry.cancelReceipt(handle)` を呼ぶ（R3-M5）
- **購読が継続している stream に対してのみ** `.finished(terminatedBy: .cancelled)` を配送してから `finish()` する
...
| OP-18 | receiveFilePromises | `receiveFilePromises(destinationDirectory:scope:policy:onEvent:) throws -> FilePromiseReceiptHandle` | **`receiveFilePromiseEvents(...) throws -> AsyncStream<FilePromiseReceiptEvent>`**（開始失敗は同期 throw、要素は非 throwing。R3-H1）と **`receiveFilePromises(...) async throws -> FilePromiseReceipt`**（H-4） |
...
| **callback が NULL（handle を返す endpoint）** | **`clipboardProvideFilePromise` / `clipboardReceiveFilePromises` は callback を必須とする。NULL の場合は登録・受領を開始せず、何も生成しない**。呼び出し側が handle を受け取れないと provider / receipt / staging を回収できなくなるため（R3-M4）。callback が NULL では通知先がないので戻り値もない — **この 2 endpoint に限り NULL callback は no-op**とする |
...
7. アプリ起動時に `ClipboardPromise/` の残骸を削除する（前回異常終了時の掃除）
...
| **IT-27** | **stream の Task キャンセルで `onTermination` から receipt が cancel され、タイマと staging が解放される。`finished` は配送されない**（R3-H1） | R3-H1 |
...
| BT-04a | **handle を返さない 17 endpoint**は callback が NULL でも操作を実行し、クラッシュしない | H-2 |
| **BT-04b** | **`clipboardProvideFilePromise` / `clipboardReceiveFilePromises` は callback NULL で登録・受領を開始しない**（資源を作らない） | **R3-M4** |
...
| **BT-11** | **§8.4.4 の全 JSON 型が round-trip する（入力 10 種・出力 10 種・イベント 2 種）** | **R2-H3 / R3-L8** |
...
| **BT-17** | **機械照合: 公開 OP が 20、Bridge endpoint が 19、§8.4.3 の prototype 数と §8.4.4 の型数が本文の記載と一致する** | **R3-L8** |
...
| T-06 | **UseCase 17 本**（+ Validator / Tracker / 集約 = ファイル 20 本）+ 単体テスト | 2.0日 | T-04, T-05 | 12.1 全通過 | 不要な `async` がないこと。**`CancelReceiveFilePromisesUseCase` を含むこと（R3-M5）** |
...
| T-11 | File Promise 提供側（delegate / registry / **`inFlightCount` + `releaseScheduled` + `generation` 状態機械** / serial queue / **staging 所有** / transaction） | 2.5日 | T-07, T-08 | IT-12〜IT-14、**IT-21〜IT-24、IT-31〜IT-34**、CT-03、CT-06、**CT-08〜CT-11** 通過 | **RK-21: coordinator が provider+delegate を強参照。completion は要求ごとに 1 回。全体で 1 回に絞るガードを置かない**。**R2-H4: `inFlightCount` で複数同時履行を扱う。R2-M5: release は完全冪等・非 throwing。R2-M12: register→write→rollback** |
| T-12 | **File Promise 受領側（静穏 + 全体タイムアウト + OP-20 cancel + `CancelReceiveFilePromisesUseCase`）** | 1.5日 | T-11 | IT-15〜IT-19、**IT-25〜IT-27、IT-30** 通過 | **H-3: `fileTypes.count` を終了判定に使っていないこと。`finished` が exactly-once。終端後の late callback 破棄**。**R2-H2: 公開 cancel。R2-M6: §7.12.3 の truth table どおり throw しないこと** |
| T-13 | OP-18 の native async（**`throws -> AsyncStream`** + 集約 async） | 0.75日 | T-12 | IT-27、IT-30 通過 | **H-4: common.md の併設要求**。**R3-H1: factory が `throws`、要素型は非 throwing、`onTermination` は資源解放専用で配送しない**。**R2-M6: 集約版の通常終端は throw せず partial receipt を返す。Task キャンセル時のみ `CancellationError`** |
...
- [x] **第 3 回レビュー指摘 8 件（高 3 / 中 4 / 低 1）をすべて反映した。反映先は §0 の対応表**
...
- [ ] **`@discardableResult` が戻り値のある OP-01〜OP-07 / OP-09〜OP-11 のみに付いている（OP-08 には付いていない）**（R3-M6）
- [ ] 12.1 の単体テストが全通過する
- [ ] 12.2 の統合テスト IT-01〜IT-20 が全通過する
- [ ] 12.3 の Presentation テスト PT-01〜PT-08 が全通過する
- [ ] 12.2 の統合テスト IT-21〜IT-34 が全通過する
- [ ] 12.4 の Bridge テスト BT-01〜BT-17 が全通過する
- [ ] 12.5 の並行性テスト CT-01〜CT-11 が全通過する
