...
final class FilePromiseLifecycleState: @unchecked Sendable {
    ...
    /// MainActor 側の最終判定。条件を満たせば isReleased = true にし staging URL を返す。
    func commitRelease(generation: UInt64) -> URL?
...
[MainActor] state.commitRelease(generation: g)
    [lock 内] releaseRequested && inFlightCount == 0
        ...
        -> stagingURL を返す（MainActor 側で削除と辞書除去）
    それ以外（新規開始で generation が進んだ等）
        -> nil を返す。MainActor は何もしない
...
| MainActor で再判定 OK | `commitRelease(generation:)` | `isReleased = true`、`scheduledGeneration = nil` | **staging URL を返す → MainActor が削除 + 辞書除去（exactly-once）** |
| MainActor で再判定 NG | `commitRelease` が `nil` → `abandonReservation(generation:)` | **`scheduledGeneration == g` のときだけ clear** | 何もしない（次の完了で再予約） |
...
public protocol ClipboardPromiseRegistry {
    ...
    /// 予約済み handle を登録する。stagingURL は事前に用意されたもの（nil 可）。
...
**transaction 順序（R3-H3 / R2-M12 / R4-H3）**

```
[MainActor] 1. handle = registry.reserveFilePromiseHandle()      // 登録しない

[background] 2. if case .snapshot(let src) = request.source {
                    staging = try await snapshotter.snapshot(from: src,
                                                             into: stagingRoot(handle))
                }  // 失敗時は部分コピーを削除して throw。登録もされていない

[MainActor] 3. registry.registerFilePromise(request, reserved: handle,
                                            stagingURL: staging)
            4. do { try repository.writeFilePromise(handle:scope:) }
...
| **Task キャンセル（集約 async）** | 該当なし | 該当なし | **`onCancel` から `Task { @MainActor }` で OP-20 へ hop（同期直呼びは actor isolation error）。`ReceiptCompletionGate` で terminal と競合させ、勝った側だけが resume する。cancel が勝てば `CancellationError`、terminal が勝てば `FilePromiseReceipt`**（R4-M4）。受領済みファイルは destination に残る | 該当なし |
...
- **`onTermination` は資源解放専用**。receipt の cancel、静穏 / 全体タイマの停止、reader / event クロージャの解除を行い、イベント配送は試みない。**提供側の staging は対象外**（R4-L9）
...
```
1. handle = registry.registerReceipt(policy:onEvent:)
2. do { try repository.startReceivingFilePromises(handle:destination:scope:) }
   catch { registry.cancelReceipt(handle)   // session / timer / closure を解放
           throw }
3. return handle（stream 版はここで subscription を組み立てる）
```
...
- `onCancel` からは **`Task { @MainActor in manager.cancelReceiveFilePromises(handle) }` で hop する**
- terminal イベント到達時と、cancel 経路の両方が `gate.claim()` を呼ぶ。**true を得た側だけが resume する**
- terminal が勝った場合 → `FilePromiseReceipt` を返す（Task がキャンセル済みでも、受領済みの結果を返す）
- cancel が勝った場合 → `CancellationError` を throw する
- どちらが勝っても内部 cleanup（receipt / timer / closure）は exactly-once で実行される
...
| 区分 | 型 | 件数 |
|---|---|---|
| 入力専用 | `ContentJson` / `OptionsJson` / `CreateRequestJson` / `MatchingTypesJson` / `FilePromiseRequestJson` / `PolicyJson` / `PatternsJson`（入力） | 7 |
| 入出力共用 | `ScopeJson` / `OwnershipJson` / `HandleJson` | 3 |
| 出力専用 | `ReadResultJson` / `ReadDataJson` / `SnapshotJson` / `ChangeCountJson` / `BoolJson` / `DetectedValuesJson` / `DetectedMetadataJson` / `AccessBehaviorJson` / `PatternsJson`（出力は入力と同形のため入力専用に計上） | 8 |
| イベント | `ChangeEventJson` / `ReceiptEventJson` | 2 |

**実体は 19 型**（入力専用 7 + 入出力共用 3 + 出力専用 7 + イベント 2。`PatternsJson` は入出力同形のため 1 件として入力専用に計上し、出力専用は 7 件）。BT-11 / BT-17 はこの数を検証する。
...
| **IT-38** | **OP-18 の cleanup が提供側の staging を削除しない**（受領済みファイルも残る） | **R4-L9** |
...
| **BT-11** | **§8.4.4 の全 JSON 型（実体 19 型）が round-trip する** | **R2-H3 / R3-L8 / R4-L8** |
...
| **BT-17** | **機械照合: 公開 OP が 20、Bridge endpoint が 19、callback 必須 endpoint が 3、§8.4.3 の prototype 数と §8.4.4 の実体 19 型が冒頭「用語」の記載と一致する** | **R3-L8 / R4-L8** |
...
| T-08 | `MacClipboardManager` 骨格（DI、callback + native、`Log.d`） | 1.0日 | T-07 | 全 OP が呼べ、callback が MainActor | **OP-01〜OP-11 と OP-16 が `async throws`。`@discardableResult` は OP-01〜07 / OP-09〜11 のみで OP-08 には付けない（R3-M6 / R4-M7）。OP-12〜OP-15 / OP-17 / OP-19 / OP-20 が同期**。CT-04 |
...
| T-11 | File Promise 提供側（delegate / registry / **`FilePromiseLifecycleState`（nonisolated lock-owning）** / serial queue / **`FilePromiseSnapshotting`（off-MainActor）** / transaction） | 3.0日 | T-07, T-08 | IT-12〜IT-14、**IT-21〜IT-24、IT-31〜IT-34、IT-39、IT-40**、CT-03、CT-06、**CT-08〜CT-12** 通過 | **RK-21: coordinator が provider+delegate を強参照。completion は要求ごとに 1 回。全体で 1 回に絞るガードを置かない**。**R2-H4: `inFlightCount` で複数同時履行を扱う。R2-M5: release は完全冪等・非 throwing。R2-M12: register→write→rollback** |
| T-12 | **File Promise 受領側（静穏 + 全体タイムアウト + OP-20 cancel + `CancelReceiveFilePromisesUseCase` + 開始 rollback）** | 2.0日 | T-11 | IT-15〜IT-19、**IT-25〜IT-27、IT-30、IT-36〜IT-38** 通過 | **H-3: `fileTypes.count` を終了判定に使っていないこと。`finished` が exactly-once。終端後の late callback 破棄**。**R2-H2: 公開 cancel。R2-M6: §7.12.3 の truth table どおり throw しないこと** |
| T-13 | OP-18 の native async（**`throws -> FilePromiseEventSubscription`** + 集約 async + `ReceiptCompletionGate`） | 1.0日 | T-12 | IT-27、IT-30、**IT-35**、**CT-13、CT-14** 通過 | **H-4: common.md の併設要求**。**R3-H1: factory が `throws`、要素型は非 throwing、`onTermination` は資源解放専用で配送しない**。**R2-M6: 集約版の通常終端は throw せず partial receipt を返す。Task キャンセル時のみ `CancellationError`** |
...
- [x] **第 4 回レビュー指摘 10 件（高 3 / 中 4 / 低 3）をすべて反映した。反映先は §0 の対応表**
...
- [ ] 12.2 の統合テスト IT-21〜IT-40 が全通過する
- [ ] 12.4 の Bridge テスト BT-01〜BT-17 が全通過する
- [ ] 12.5 の並行性テスト CT-01〜CT-14 が全通過する
