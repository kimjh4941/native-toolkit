# macOS Clipboard サンプルアプリ実装計画 v3
...
| 中: 機能設計 §2.1 の U-05〜U-07 が計画に不在 | **§1.5 に対象外の理由を明記** |
...
| `partialPasteContent` | **MT-06 用**。accepted type に合う item と合わない item を混ぜた 2 item。Paste Control で部分失敗の表示を作る |
...
`ClipboardError` は 20 ケースある。**サンプルから到達できるものとできないものを分ける。**

| 分類 | コード | 扱い |
|---|---|---|
| **専用ボタンで到達**（8） | 1501 / 1503 / 1504 / 1508 / 1511 / 1512 / 1515 / 1523 | §6.1 の `(expect error)` |
| **環境依存で到達**（2） | 1513（macOS 15.4 未満）、1514（ユーザーが拒否） | 到達したら Error Cases に記録 |
| **到達手段なし**（10） | 1502 / 1505 / 1506 / 1507 / 1509 / 1510 / 1521 / 1522 / 1524 / 1599 | サンプルからは作れない。ライブラリの単体テストが担保 |

**「全 20 ケースを画面から出す」ことは目標にしない。** 1506（サイズ超過）は数百 MB の
確保が要り、1509 / 1510（pasteboard が書き込みを拒否）は再現手段がない。到達できない
ものを無理に作ると、サンプルが機能の実演ではなく異常系の再現装置になる。

## 7. 検証（M-8）

### 7.1 自動テスト

`MacLibraryExampleTests` に置く。
...
| **ST-04** | 同 formatter の出力が、値・完全パス・query・pasteboard 名を含まない | 機微な入力を与えて出力を検査 |
...
#### ST-03 / ST-04 を書けるようにする設計（レビュー中 6）
...
右辺: ClipboardSampleView.swift から `MacClipboardManager.shared.<name>(` を
      抽出した関数名の集合
