...
| L-8 | 9 content kind / 4 loaded item kind のサイズ算定規則 | UTF-8 byte、file size、input/output Data、pixel count の各検査点を定義（S2 / S6） |
...
| D-14 | 非協調的 async System API の cancel / timeout | `ClipboardAsyncRaceCoordinator` が caller を即時完了し、OS の遅延結果だけを破棄 | structured task group は cancel 後も child 完了を待つ可能性があるため |
| D-15 | 同期 control / factory の Manager 公開方式 | P-12〜P-16 は同期形式を維持し、T-R で `ios.md` に明示的な例外を追加 | System API と下位層を不必要に async 化せず、現行 `ios.md` との不整合を正式解消するため |
| D-16 | `@MainActor` 型の cleanup | Manager / Loader / Container は `isolated deinit` を使用 | 通常 `deinit` から actor-isolated method を呼ぶと Swift 6 strict で失敗するため |
| D-17 | custom UTI の検証 | custom data は未登録 reverse-DNS identifier を許容し、image data だけ既知 image UTI を要求 | `UTType(_:)` は未登録 custom identifier に nil を返すため |
| D-18 | 公開 error message の秘匿 | message は入力値を含まない固定文、値は非公開 diagnostic detail のみ | URL / path / pasteboard name のログ再流入を防ぐため |
...
| 重い同期処理 | `UIImage.pngData()` / `UIImage(data:)` | 即時だが CPU 負荷大 | **background executor へ移し、呼び出し側には非同期として公開**。P-1 / P-2 は最終 `setItems` / `addItems` 自体は sync だが、content enum が画像を含みうるため Repository / UseCase を `async throws` とする |
...
- `IosClipboardManager.init(timeouts:limits:)` で差し替え可能（テストで短縮するため）。`.default` は private unchecked initializer から既知の正値で生成する
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| P-1 | `copy` | 画像前処理 = heavy sync、`setItems(_:options:)` = sync | A `throws`（画像coderをawait後に同期write） | A `throws` | C | A | `clipboardCopy` | Repository / UseCase / Manager は`@MainActor`、画像coderのみbackground | 画像kindのみ`imageCoding` = 10s | native Task cancelは`.cancelled`を即時throw。callback版は明示cancelなし | なし | content enumが画像を含みうるため、System writeはsyncでも前処理込みの操作をasync公開 |
| P-2 | `append` | 画像前処理 = heavy sync、`addItems(_:)` = sync | A `throws`（同上） | A `throws` | C | A | `clipboardAppend` | 同上 | 画像kindのみ`imageCoding` = 10s | 同上 | なし | 同上 |
...
| P-12 | `cancelAllLoads` | `Progress.cancel()` = sync | S | S `CancelAllLoadsUseCase` | S | S | `clipboardCancelLoads` + operation callback | `@MainActor` | - | 各pendingへ`.cancelled` | 未配信の一時ディレクトリは Loader が削除 | Managerは同期。Bridgeは任意thread→main hop完了をcallbackで通知 |
...
| P-14 | `stopObserving` | `removeObserver` = sync | S（Manager所有） | - | S | S | `clipboardStopObserving` + operation callback | `@MainActor` | - | - | なし | Managerは同期。Bridgeはmain hop完了をcallbackで通知 |
| P-15 | `checkForegroundChange` | `changeCount` = sync | S | S `CheckForegroundChangeUseCase` | S（`Bool` 返却） | S | `clipboardCheckForegroundChange` | `@MainActor` | - | 不可 | trackerはUseCase所有 | 同期完結のためManagerはcallback化しない。BridgeのみJSON callback |
| P-16 | `makePasteControl` | `UIPasteControl` 生成 = sync、貼り付け = UI callback | Presentation `PasteItemProviderLoader`（C + token） | - | S（生成） + C（貼り付け結果） | S | **非公開** | `@MainActor` | `providerLoad` / `imageCoding` | コンテナ `isolated deinit` / 次回 paste 開始時。**キャンセル時は UI callback を呼ばない**（D-4） | `.file` 成功時のみ呼び出し側所有。未配信分は Presentation が削除 | UIKit 型のため Bridge 非公開 |
...
5. actor hop は Bridge、画像コーディングの background executor、非協調的async処理のrace coordinatorに限定する
6. P-1 / P-2 は最終System writeがsyncでも、画像kindのbackground前処理を待つためRepository / UseCaseをasyncとする。非画像kindは同じpublic enum経路を共有するが、不要なbackground hopは行わない
7. **P-12〜P-16 は同期完結の control / factory API のため callback / async 版を設けない**。この例外はT-Rで`ios.md`へ正式に追記する。P-12 / P-15はData Portへ触れるためUseCaseを持ち、P-13 / P-14はManagerのlistener所有、P-16はPresentation factoryとしてUseCaseを持たない
...
| Manager は UseCase 経由で Data にアクセス | 適合 | P-1〜P-12とP-15に13個のUseCaseを用意。P-13 / P-14はManagerのlistener所有、P-16はPresentation factoryとしてDataへ直接アクセスしない |
| system Delegate / Listener の所有は Manager 層の 1 クラス | 適合 | `NotificationCenter` トークンは `IosClipboardManager` のみ |
| Manager は callback 版 + `async throws` 版を併設 | **T-R完了後に適合** | P-1〜P-11は併設。P-12〜P-16は即時control / factoryとして同期形式を維持し、この例外を`ios.md`へ追加する |
...
| **Domain は標準ライブラリのみ** | **案Aを採用、T-R完了後に適合** | platform UIに依存しない`Sendable`なFoundation値型だけを限定許容 |

### Domain の `Foundation` 依存（案Aを採用・T-Rでcanonical ruleへ反映）

本設計は案Aを採用する。既存Share / NotificationのDomainもFoundation値型を利用しており、`[UInt8]` / epoch / path文字列へ変換する案Bは変換量を増やして型安全性を下げるため採用しない。

T-Rで`common.md`のDomain定義を次の条件へ改訂する。

- `Data` / `Date` / `URL` / `UUID`など、platform UIに依存せず`Sendable`なFoundation値型は許容する
- `UIKit` / `AppKit`、system service object、delegate、platform API objectは許容しない
- `NSError`や任意のsystem `Error`をDomainへ保持しない
- 新たなFoundation型をDomain public APIへ追加する場合は、値型・`Sendable`・UI非依存を設計レビューで確認する

同じT-Rで、`ios.md`のManager公開規約に「即時完了する同期control / factory APIは、callback / async wrapperを設けず同期形式を維持してよい。待機または結果の非同期到着を伴う操作はcallback + `async throws`を必須とする」を追加する。**T-R完了前はcanonical rule未反映のため実装を開始しない。**
...
- **公開メッセージは入力値を含まないerror caseごとの固定英語文**（locale / OS / provider に依存しない）
...
| Manager は callback 版 + `async throws` 版 | T-R完了後に適合 | 同期control / factory例外を`ios.md`へ正式追加する |
...
`.m` からは `#import <IosLibrary/IosLibrary-Swift.h>` 済みのため `[ClipboardRedaction json:nsJson]` として呼べる。**モジュール境界の成立を U-114 とビルドテスト I-09 で検証する。**
...
| `Application/UseCase/CancelAllLoadsUseCase.swift` | Application | P-12（Loader Port経由） |
| `Application/UseCase/CheckForegroundChangeUseCase.swift` | Application | P-15（Repository + tracker） |
...
| `Data/Concurrency/ClipboardAsyncRaceCoordinator.swift` | Data | 非協調的async処理のcancel / timeout即時完了（D-14） |
...
[ClipboardUseCases]  @MainActor                    ← Application 層
    ...
    │  CreatePasteboard / RemovePasteboard / DetectPatterns / DetectValues / LoadItem /
    │  CancelAllLoads / CheckForegroundChange（13 個）
...
[ClipboardRepositoryImpl] [ClipboardItemLoaderImpl] [ClipboardTypeIdentifierValidator] [SystemClock]
    ...
    │  + TemporaryFileStore / ImageCoder（background）/ CancellationBox / AsyncRaceCoordinator
    ▼
...
| `ClipboardAsyncRaceCoordinator<Value>` | `final class`, `@unchecked Sendable`（内部ロック） | callerのcancel / timeout / System完了を1回だけresolve |
...
**cleanup**: `IosClipboardManager` / `ClipboardItemLoaderImpl` / `PasteItemProviderLoader` / `ClipboardPasteControlContainerView` は、actor-isolatedな停止・キャンセルを行うため`isolated deinit`を明示する。通常`deinit`は使用しない。I-10で全型をSwift 5 / Swift 6 modeのstrict concurrency対象にする。
...
### 非協調的 async API の race coordinator（D-14）

`detectedPatterns` / `detectedValues` と background image coding は、Task cancellationを受けても処理自体が停止する保証がない。structured task groupでtimeoutと競争させると、scope終了時に未完了childを待つ可能性があるため使用しない。

`ClipboardAsyncRaceCoordinator<Value>`はlockで次を1回だけresolveする内部helperとする。

| 到着元 | callerへの結果 | 背後処理 |
|---|---|---|
| System / image処理の完了 | 値またはerrorをresume | timeout taskをcancel |
| native Task cancel | `ClipboardError.cancelled`を即時resume | OS / detached処理は継続可。遅延結果を破棄 |
| timeout | `ClipboardError.timedOut(operation:)`を即時resume | OS / detached処理は継続可。遅延結果を破棄 |

- coordinatorは`withCheckedThrowingContinuation`を保持し、attach前cancelもflagで回収する
- System呼び出しとtimeoutは**unstructured task**として開始し、coordinatorの完了はそれらの終了を待たない
- cancel / timeout / System完了の全経路が同じ`resolveOnce`を通る
- coordinator完了後に届いた値、error、生成Dataは公開せず破棄する。一時ファイルがあれば所有層が削除する
- P-9 / P-10はUseCase、P-1 / P-2の画像前処理とP-11 / P-16のimage codingはImageCoder / Loaderがcoordinatorを所有する
...
- **`append` は `localOnly` / `expirationDate` を保証しない**。DocC・Bridge HeaderDoc・API制約表の3箇所に明記する
...
**copy / append実行方式**: `CopyContentUseCase` / `AppendContentUseCase` と対応Repository methodは`async throws`とする。非画像kindは検証後に同じmain actor turnで同期writeし、background hopしない。`imageData` / `imageFile`だけ`ClipboardImageCoder`をawaitしてからmain actorへ戻り、`setItems` / `addItems`を同期実行する。native Task cancelまたはimage timeout時はpasteboardへ書き込まない。
...
| kind別算定表の合計が `limits.maxCopyByteCount` 超 | 拒否 | `contentTooLarge` |
...
| ケース | エラー |
|---|---|
| 標準UTIが`UTType(_:)`で解決できない | `invalidTypeIdentifier` |
| `imageData`のUTIが既知でない、または`UTType.image`にconformしない | `invalidTypeIdentifier` |
| `customData` / `multiRepresentation`のidentifierが既知標準UTIでも有効なreverse-DNS custom identifierでもない | `invalidTypeIdentifier` |

UseCaseがPort経由で事前検証するため、`UTType`はApplicationに現れない（D-12）。custom identifierはASCII英数字から始まるdot区切りを基本とし、各segmentで英数字・`-`・`_`を許容する。`com.jonghyunkim.nativetoolkit.custom-payload`はhost appで未登録でも成功させる。標準UTIは`UTType(_:)`を先に試すため、reverse-DNS独自判定による誤拒否をしない。
...
**copy / appendのサイズ算定（L-8）**

| content kind | `maxCopyByteCount`の算定 |
|---|---|
| `plainText` / `url` | UTF-8 byte count |
| `htmlText` | plain + htmlのUTF-8 byte count |
| `multipleText` | 各文字列のUTF-8 byte count合計 |
| `customData` / `imageData` | `Data.count` |
| `multiRepresentation` | identifier UTF-8 byte + value `Data.count` の全entry合計 |
| `imageFile` | resource valueのfile sizeを読み、decode前に検査。decode後はpixel count、encode後は出力`Data.count`も検査 |
| `color` | 固定32 byteとして算定 |

合計は`addingReportingOverflow`で計算し、overflowも`contentTooLarge`とする。画像は入力byte、`maxImagePixelCount`、出力byteの3点をすべて満たす必要がある。
...
| Manager callback | `readData(utType:scope:completion: (Bool, Data?, String?, String?) -> Void)` |
| Manager native | `readData(utType:scope:) async throws -> Data?` |
| Bridge | `clipboardReadData` |
| テスト | U-30（UseCase 正常）、U-31（UTI 不正で Repository 未呼び出し）、U-32（該当なし → nil）、U-52（Data実体）、U-100（Manager）、U-124（Bridge schema） |
...
7. `cancelAll()` / `isolated deinit` / token `cancel()` / タイムアウトは、それぞれ `.cancelled` / `.timedOut(operation:)` を **1 回**配信してから管理表から除去する
...
| loaded kind | 検査点 |
|---|---|
| text / URL | Domain文字列へ変換直後のUTF-8 byte count |
| image | providerから受けたinput Dataまたはobject表現の推定byte、decode後pixel count、PNG encode後`Data.count` |
| file | security-scoped access開始後・一時copy前にresource file sizeを取得。copy後にも実sizeを再検査 |

いずれかが`maxLoadByteCount`または`maxImagePixelCount`を超えた場合は公開前に`contentTooLarge`とし、作成済み一時領域を削除する。providerからData / UIImageを受け取った時点のメモリ割り当て自体はSystem APIのため事前抑止できず、上限は「受入・変換・公開を拒否する契約」であることをDocCへ記載する。
...
- `ClipboardItemLoaderImpl` / `PasteItemProviderLoader`は`isolated deinit`でpending requestをcancelする（D-16）
...
`CheckForegroundChangeUseCase`（`@MainActor`）が`ClipboardRepository.changeCount(scope:)`とtrackerを所有し、`execute(scope:) -> Bool`、監視開始時の`resync(scope:)`、通知受信時の`markReported(scope:)`を提供する。これらはP-15と同じforeground差分状態を維持するための補助入口であり、ManagerはRepositoryを直接呼ばない。`CancelAllLoadsUseCase.execute()`は`ClipboardItemLoader.cancelAll()`へ委譲し、P-12でもManagerからLoaderを直接呼ばない。
...
`startObserving` は既に監視中なら先に `stopObserving()`。`IosClipboardManager`の`isolated deinit`でも`stopObserving()`と`CancelAllLoadsUseCase.execute()`を呼ぶ。
...
SDK上でnonoptionalな`probableWebURL` / `probableWebSearch`は、対応key pathが`DetectedValues.patterns`に含まれず値が空文字の場合にDomainの`nil`へ正規化する。patternが含まれる場合は空文字でも検出値として保持する。この規則をMapper testで固定する。
...
**キャンセル / タイムアウト**: UseCaseが`ClipboardAsyncRaceCoordinator`を所有する。native Task cancelは`cancelled`、`timeouts.detection`（5秒）超過は`timedOut(operation: .detection)`をcallerへ即時throwする。OS taskの終了は待たず、遅延結果だけを破棄する。callback Manager / Bridgeには同じDomain errorを1回だけ変換して返す。
...
public final class ClipboardPasteControlContainerView: UIView {
    ...
    public init(acceptedTypes: [String], displayMode: UIPasteControl.DisplayMode) throws
    // isolated deinit: cancels pending loads and removes undelivered temporary directories
}
```
...
- したがってエラー表の `cancelled` の発生元は「内部 completion」であり、UI へは伝播しない。U-84 / U-90 はこの契約を検証する
...
- `acceptedTypes`は1件以上必須。空配列、空identifier、不正identifierは`invalidRequest` / `invalidTypeIdentifier`としてcontrol生成前にthrowする。M-10の「未設定」は空配列拒否を確認する
...
Operation callbackのtruth table:

| 結果 | `isSuccess` | `errorCode` | `errorMessage` |
|---|---:|---|---|
| 成功 | `true` | NULL | NULL |
| `ClipboardError` | `false` | 非NULL固定code | 非NULL固定message |
| 未知error | `false` | `CLIPBOARD_UNKNOWN` | `An unknown error occurred.` |
...
| kind | 必須 | 任意 / nullable |
|---|---|---|
| `changed` | `kind`, `scope`, `typesAdded`, `typesRemoved` | なし。両配列は空可 |
| `changedDetectedOnForeground` | `kind`, `scope` | なし |
| `removed` | `kind`, `scope` | なし |

eventは成功envelopeで包まず、上表のobjectを`ClipboardChangeCallback`へ直接渡す。開始時errorはevent callbackへ流さず、`startCallback`だけへ返す。
...
**error details**: `providerLoadFailed` / `fileCopyFailed` / `detectionFailed` / `unknown`だけが`{"domain": string, "code": int}`を任意で持つ。入力URL / path / pasteboard name / invalid reasonはdetailsにも含めない。
...
`.m` の完了ブロック引数は `BOOL` / `NSString * _Nullable`。全 C 関数の先頭で `[ClipboardRedaction json:...]` を通したログを出す。P-12 / P-14はManager上では同期だが、任意スレッドのC callerからmain actorへhopするためBridgeではoperation callbackを完了境界とする。callback後はcancel / stopが完了済みであり、`clipboardStopObserving` callback後に旧eventを配信しない。
...
| P-3 | `read(scope:completion: (Bool, ClipboardReadResult?, String?, String?) -> Void)` | `read(scope:) async throws -> ClipboardReadResult` |
| P-4 | `readData(utType:scope:completion: (Bool, Data?, String?, String?) -> Void)` | `readData(utType:scope:) async throws -> Data?` |
| P-5 | `snapshot(matchingTypes:scope:completion: (Bool, ClipboardSnapshot?, String?, String?) -> Void)` | `snapshot(matchingTypes:scope:) async throws -> ClipboardSnapshot` |
| P-6 | `clear(scope:completion:)` | `clear(scope:) async throws` |
| P-7 | `createPasteboard(_:completion: (Bool, PasteboardScope?, String?, String?) -> Void)` | `createPasteboard(_:) async throws -> PasteboardScope` |
| P-8 | `removePasteboard(_:completion:)` | `removePasteboard(_:) async throws` |
| P-9 | `detectPatterns(_:scope:completion: (Bool, Set<ClipboardDetectionPattern>?, String?, String?) -> Void)` | `detectPatterns(_:scope:) async throws -> Set<ClipboardDetectionPattern>` |
| P-10 | `detectValues(_:scope:completion: (Bool, ClipboardDetectedValues?, String?, String?) -> Void)` | `detectValues(_:scope:) async throws -> ClipboardDetectedValues` |
| P-11 | `loadItem(_:scope:completion: (Bool, ClipboardLoadedItem?, String?, String?) -> Void) -> any ClipboardLoadToken` | `loadItem(_:scope:) async throws -> ClipboardLoadedItem` |
...
| P-16 | `makePasteControl(acceptedTypes:displayMode:onPaste:onPartialFailure:onPasteFailure:) throws -> ClipboardPasteControlContainerView` |

値付きcallbackは成功時`(true, value, nil, nil)`、失敗時`(false, nil, errorCode, errorMessage)`に統一する。値なしcallbackは成功時`(true, nil, nil)`、失敗時`(false, errorCode, errorMessage)`とする。
...
| `ClipboardCancellationBox` / `ClipboardAsyncRaceCoordinator` | `internal` | cancellation / timeoutの内部実装。public APIへ露出しない |
| `ClipboardChangeTracker` / `ClipboardContentValidator` | `public` | 純ロジックの単体テスト対象 |
| UseCase 群（13 個） | `public` | P-1〜P-12およびP-15を担当し、既存 `ShareContentUseCase` に合わせる |
...
| `pasteboardUnavailable(name)` | `CLIPBOARD_UNAVAILABLE` | `The requested pasteboard is unavailable.` | 共通 |
...
| `invalidURL(v)` | `CLIPBOARD_INVALID_URL` | `The URL is invalid.` | iOS 固有 |
| `invalidTypeIdentifier(v)` | `CLIPBOARD_INVALID_TYPE` | `The uniform type identifier is invalid.` | iOS 固有 |
| `invalidPasteboardName(v)` | `CLIPBOARD_INVALID_NAME` | `The pasteboard name is invalid.` | iOS 固有 |
...
| `invalidRequest(reason)` | `CLIPBOARD_INVALID_REQUEST` | `The request is invalid.` | iOS 固有 |
| `contentTooLarge(b, l)` | `CLIPBOARD_CONTENT_TOO_LARGE` | `The clipboard content exceeds the configured size limit.` | iOS 固有 |
| `fileNotFound(path)` | `CLIPBOARD_FILE_NOT_FOUND` | `The requested file was not found.` | iOS 固有 |
| `imageLoadFailed(path)` | `CLIPBOARD_IMAGE_LOAD_FAILED` | `Failed to load the image.` | iOS 固有 |
...
| `timedOut(op)` | `CLIPBOARD_TIMED_OUT` | `The clipboard operation timed out.` | iOS 固有 |
...
| U-02 | 〃 | 正常 | 全24ケースの`errorDescription`が固定英語文で、system message・URL・path・pasteboard name・invalid reasonを含まない |
...
| ID | 対象 | 系 | ケース |
|---|---|---|---|
| U-132 | Copy / Repository | 正常 | 非画像kindはbackground hopなし、画像kindはmain外で前処理後にmainで`setItems` |
| U-133 | Append / Repository | 正常 | 画像kindをmain外で前処理後にmainで`addItems` |
| U-134 | Copy / Append | 異常 | native Task cancelで`.cancelled`を即時throwし、pasteboardへ書き込まない |
| U-135 | ImageCoder race | 異常 | timeoutを即時返し、遅延結果を破棄してpasteboardへ書き込まない |
| U-136 | Detection race | 異常 | Task cancelを即時返し、OS遅延結果を破棄 |
| U-137 | Detection race | 異常 | timeout後にOS task完了を待たず即時復帰 |
| U-138 | MainActor cleanup | 正常 | Manager / Loader / Containerが`isolated deinit`でstrict concurrencyを通る |
| U-139 | UTI validator | 正常 | 未登録`com.jonghyunkim.nativetoolkit.custom-payload`をcustom dataで許容 |
| U-140 | UTI validator | 異常 | `imageData`は未知UTIまたは非image UTIを拒否 |
| U-141 | Size accounting | 境界 | 9 content kindすべてでbyte算定・overflow・上限ちょうどを検証 |
| U-142 | Load limits | 境界 | text / URL / image / fileの入力・出力上限とcleanupを検証 |
| U-143 | Timeout / Limits init | 境界 | 0、負数、非有限timeoutを拒否し、正値を保持 |
| U-144 | Detection mapper | 境界 | nonoptionalな空文字をpattern有無の規則でnil / 空文字へ変換 |
| U-145 | Control UseCases | 正常 | P-12 / P-15がUseCase経由でLoader / Repositoryを呼び、Managerは直接呼ばない |
| U-146 | Manager callback | 正常 | 値付きcallbackのBool・value・error nil条件がtruth tableと一致 |
| U-147 | Paste control | 異常 | acceptedTypes空 / 不正identifierをcontrol生成前にthrow |
| U-148 | Bridge control | 正常 | cancel / stopのoperation callback後に処理が完了し、stop後eventは届かない |
...
|---|---|---|---|---|---|
| **T-R** | **canonical rule改訂**: `common.md`へ限定Foundation値型許容、`ios.md`へ同期control / factory例外を追加 | 0.5日 | なし | 本設計213節の文言が反映され、Domain依存とP-12〜P-16の公開方式が正式ルール化 | 既存Shareとの整合、UI / system object除外、非同期操作を例外化していないか |
...
| T-03 | Port 4 種（actor 宣言）+ UseCase 13 種 + 集約 + Mock 4 種 | 2日 | T-02 | U-23〜U-49、U-145がgreen。Portと実装のisolationが一致 | DetectValues / CancelAllLoads / CheckForegroundChangeの存在、UTI Port、Manager直アクセス禁止 |
| T-04 | `PasteboardResolver` / `TypeIdentifierValidator` / `Mappers` / `DetectionMapper` / `ClipboardRepositoryImpl` | 2.5日 | T-03 | U-50〜U-65、U-132〜U-144がgreen。`setValue`不使用 | 画像copyのasync前処理、custom UTI、全kindサイズ、検出race / Mapper |
| T-05 | `TemporaryFileStore` / `ImageCoder` / `CancellationBox` / `AsyncRaceCoordinator` | 2日 | T-01 | U-66〜U-79、U-135〜U-143がgreen | path traversal、cleanup、即時cancel / timeout、background実行、`Sendable` |
...
| T-07 | `IosClipboardManager`（`@MainActor`、P-1〜P-16、監視トークン所有、Task キャンセル転送） | 2.5日 | T-04, T-06 | U-97〜U-112、U-134、U-145〜U-146がgreen | actor境界、callback truth table、同期control、UseCase経路、continuation 1回 |
| T-08 | Presentation（`PasteItemProviderLoader` → `ReceiverView` → `ContainerView` → `Factory`） | 2日 | T-06 | U-80〜U-96、U-138、U-147がgreen | 集約契約、`isolated deinit`、acceptedTypes検証、receiver保持 |
| T-09 | `UnityIosClipboardJsonParser`（15 endpoint の完全 schema） | 2日 | T-01 | U-116〜U-127 が green | 全 union の必須 / 任意 / nullable、未知 kind、秘匿ログ |
| T-10 | `UnityIosClipboardManager` + Bridge `.h` / `.m`（errorCode透過 + generic runner + control完了callback） | 1.5日 | T-09, T-07 | U-128〜U-131、U-148がgreen。ブロック引数が`BOOL` | Bridgeの薄さ、C文字列寿命、NULL、cancel / stop完了順序、HeaderDoc |
...
| R-11 | Domain の `Foundation` 依存が現行`common.md`と不整合 | ルール逸脱 | **案A採用済み**。限定Foundation値型許容をT-Rで反映してからT-01開始 | T-R待ち（ブロッカー） |
...
| R-15 | 画像copy/appendの前処理を同期writeと混同する | main thread負荷または実装不能 | Repository / UseCaseをasync化し、画像だけbackground前処理後にmainでsync write。U-132〜U-135 | 緩和済み |
...
- [ ] T-Rで`common.md`の限定Foundation値型許容と`ios.md`の同期control / factory例外が反映されている
...
- [ ] 同期 System API に対応する Repository / UseCase は、P-1 / P-2の画像前処理を除き`async`化されていない
- [ ] P-1 / P-2だけは画像前処理を待つ明示理由でRepository / UseCaseがasyncであり、非画像kindはbackground hopしない
- [ ] actor-isolated cleanupを持つ型が`isolated deinit`を使用する
...
- [ ] 単体テスト U-01 〜 U-148 が全て green（Swift Testing、XCTest 不使用）
...
| 項目 | 内容 | 確認タイミング |
|---|---|---|
| Canonical rule反映 | 採用済み案Aの限定Foundation値型許容と、同期control / factory例外をrulesへ反映 | **T-R（実装のブロッカー）** |
...
| 優先度 | 指摘 | 反映内容 |
|---|---|---|
| 高 | 画像copy/appendの同期UseCaseとbackground前処理が両立しない | P-1 / P-2のRepository / UseCaseをasync化。非画像はhopせず、画像だけImageCoderをawait後にmainで同期write。timeout / Task cancel / 書込抑止を追加 |
| 高 | 検出APIのcancel / timeoutでcallerを即時完了できない | `ClipboardAsyncRaceCoordinator`を追加。unstructured System taskを待たず、cancelは`.cancelled`、timeoutは`.timedOut`を即時resumeし遅延結果を破棄 |
| 高 | `@MainActor`型の通常deinit cleanupがstrict build不可 | Manager / Loader / Presentation loader / Containerに`isolated deinit`を指定し、U-138 / I-10を追加 |
| 高 | P-12〜P-16が`ios.md`のManager公開規約と不整合、値付きcallbackにBoolがない | T-Rで同期control / factory例外を`ios.md`へ正式追加。P-1〜P-11の値付きcallbackを`(Bool, value?, code?, message?)`へ統一 |
| 高 | P-12 / P-15のUseCase経路がない | `CancelAllLoadsUseCase` / `CheckForegroundChangeUseCase`を追加し、ManagerのLoader / Repository直アクセスを排除 |
| 高 | `UTType(_:)`が未登録custom UTIを拒否 | custom dataは既知標準UTIまたは有効reverse-DNS identifierを許容し、imageDataだけ既知image適合を必須化。指定identifierの回帰testを追加 |
| 中 | 全kindへのサイズ上限適用が不完全 | 9 content / 4 loaded kindのUTF-8・file・Data・pixel算定表とoverflow検査を追加 |
| 中 | Timeouts / Limitsのpublic initとvalidationがない | failable public initとprivate unchecked default生成を定義 |
| 中 | probableWebURL / Searchのoptional変換がない | detected pattern有無と空文字に基づくnil変換規則を追加 |
| 中 | event schema / callback nil条件 / error detailsが省略 | change event union、operation truth table、details対象caseを追加 |
| 中 | 公開messageへ入力値を埋め込み、ログ再流入しうる | 全messageを入力値なし固定文へ変更し、値は非公開diagnostic detailに限定 |
| 中 | acceptedTypes空 / 不正時が未定義 | P-16を同期throwsとし、control生成前に検証する契約とU-147を追加 |
| 中 | Bridge同期controlの完了順序がない | cancel / stopにoperation callbackを追加し、callback後完了・stop後非配信をU-148で固定 |
| 中 | CancellationBoxが不要にpublic | CancellationBox / AsyncRaceCoordinatorをinternalへ限定 |
| 低 | 本文のtest ID参照が不一致 | logger / readData / UI cancel参照を現行IDへ修正し、第3回回帰test U-132〜U-148を追加 |
| 低 | append非保証をerror表へ記載するとしつつ記載なし | 制約の記載先をDocC / Bridge HeaderDoc / API制約表へ統一 |
| 判断 | R-11 Foundation依存 | 案Aを採用。platform UI非依存・SendableなFoundation値型だけを許容する限定文言をT-Rへ確定 |
