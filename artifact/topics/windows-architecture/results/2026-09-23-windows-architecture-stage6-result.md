...
## 6. 追補: C ABI の節を全操作に広げた（2026-09-23）

F-3 では「C++ API を本文、C ABI を節」とし、C ABI は代表例 1〜2 個と対応表だけにしていた。利用者の指摘で見直した。**1.11.0 のマニュアルは Windows の全操作に C の例が載っていた**（当時の Windows の章が C ABI そのものだった）ため、そのままでは C や他言語から使う人にとって後退になる。

書き方は、操作ごとに見出しを立てるのではなく、**機能グループごとに 1 つのコードブロックへまとめた**。全操作がコードとして 1 回ずつ登場し、`memset` → `struct_size` → 呼ぶ → `_free` の定型の繰り返しは避けられる。

| 章 | C ABI のコードブロック | 含む操作 |
|---|---|---|
| dialog | 3（メッセージボックス、ファイルの選択、フォルダーの選択） | 6 |
| notification | 4（ランタイムとマネージャーと活性化、内容の組み立てと表示・予約、進捗とバッジと設定、一覧と削除） | 14 |
| clipboard | 5（セッションとリスナーと終了、書き込み、読み取りと検査、遅延レンダリング、履歴） | 27 |

確認:

| 確認 | 結果 |
|---|---|
| 公開関数の網羅 | `.def` の **105 関数すべて**がいずれかの例に登場する |
| コンパイル | 例を機械的に抜き出して C（`/TC`、/W4、警告をエラー扱い）でコンパイル。**警告 0 で成功** |
