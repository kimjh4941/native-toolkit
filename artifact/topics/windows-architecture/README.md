...
| C ABI | 公開する関数の一覧。D-6 〜 D-10 の決定の反映。メモリの確保と解放の決まり。**今の 52 関数と新しい関数の対応表**（下記）。**関数名の規則**、**3 つの受け渡し方式の使い分け**、**機能ごとのスレッドの前提**（下記） |

C ABI の設計書には、冒頭に次の 3 つを置く。D-6 〜 D-10 はどれも C ライブラリの定番の作法（ハンドルは SQLite / libgit2、バッファとサイズ問い合わせは Win32、`void* user_data` は libcurl / libuv、戻り値でのエラーは libgit2）に沿っているが、そのままでは利用者が迷う点が残るためである。

| 書くこと | 理由 |
|---|---|
| 関数名の規則（`ntk_<機能>_<動詞>` の形。D-6 の例に合わせる） | 今は例示だけで規則が決まっていない |
| 3 つの受け渡し方式の使い分け（バッファ + サイズ問い合わせ / 不透明ハンドル / `cbSize` 付き構造体） | 種類ごとに最適なものを選んだ結果、**利用者は 3 つの作法を覚えることになる**。どれをいつ使うかを明示する |
| 機能ごとのスレッドの前提 | Clipboard はオーナーが STA でメッセージポンプを回し、履歴はフォアグラウンドを要求する。**コンソールのスクリプトから履歴を使うような形は成立しない**。汎用の C ABI と謳う以上、この制約は目立つ場所に要る |

**段階 5 で `agent-rules/coding-rules/windows.md` を直す。** 今の `windows.md` は C ABI（`extern "C"` + JSON + `DWORD* pError`）と公開の singleton を前提に書かれており、段階 3 の間は C ABI が DLL ブリッジとして残るため正しいまま据え置いた。直す節と変更案は C++ API の設計書 5.2 にある（Bridge の置き場所、公開面の singleton、JSON を渡す原則）。**段階 5 の C ABI 設計書と同じ PR で反映する。**

C ABI の設計書には、**今の 52 関数と新しい関数の対応表**を含める。`unity-native-plugin` は別リポジトリなので、対応表がないと向こうでの P/Invoke の書き換えが手探りになる。段階 5 で変わるのは D-6 〜 D-10 のすべて（JSON の廃止、UTF-8 化、固定幅の型、`void* user_data` の追加、戻り値でのエラー返し）であり、**P/Invoke 層は実質すべて書き直しになる**。対応表はこちらが出し、書き換え自体は `unity-native-plugin` 側の作業とする。
...
| バッジのグリフの種類 | FlaUI | グリフはアイコン用フォントの文字として読め、alert は `U+EDAD`。当初は computer use としていたが、段階 0c で文字コードで確かめられると分かった（UI テストの設計書の U-7） |
