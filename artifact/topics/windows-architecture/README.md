...
| 段階 | 内容 | 動作の変更 | ブランチ | PR |
|---|---|---|---|---|
| 0a | スパイク: FlaUI でトーストとバッジを扱えるかを確かめ、computer use で補う項目を確定する（7.2） | 無し | - | - |
...
| 項目 | 手段（見込み） | 確認すること |
|---|---|---|
| Dialog（`MessageBox`、ファイル・フォルダのダイアログ） | FlaUI | 標準の Win32 ウィンドウなので、見つけてボタンを押し、サンプルに表示された戻り値を確かめる |
| Notification の API の戻り値とサンプルの表示 | FlaUI | 他の画面と同じ |
| トーストの表示 | FlaUI（段階 0a で確認） | デスクトップ全体からトーストを探し、タイトルと本文を確かめる。ライブラリの `getAllNotifications` でも表示されたことを確かめる |
| トーストのボタンを押したときのコールバック | FlaUI（段階 0a で確認） | トーストのボタンを押し、サンプルに表示された `argsJson` を確かめる |
| スケジュール通知 | FlaUI（段階 0a で確認） | 数秒後に表示されるよう予約し、待ってから確かめる |
| バッジ | **未確定**（段階 0a で確認） | タスクバーのアイコンのバッジが UI Automation から読めない場合は computer use にする |

段階 0a で確かめること:

- Windows 11 の日本語表示で、トーストを UI Automation で見つけられるか（名前や構造がバージョンや表示言語で違う可能性がある）
- トーストが表示されるまでの遅延と、自動で消えるまでの時間
- 「応答不可」（集中モード）が有効だとトーストが表示されない。テストの前に確かめる仕組みが必要か
- バッジが UI Automation から読めるか
...
- [ ] D-1 〜 D-10 を決める
- [ ] チケットを割り当てる
- [ ] 段階 0a のスパイクで、FlaUI と computer use の分担を確定する
