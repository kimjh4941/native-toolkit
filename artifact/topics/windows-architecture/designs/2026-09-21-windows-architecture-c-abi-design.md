...
| 対象 OS | Windows 11 以降 |
| 作成日 | 2026-09-21（v1 のレビューを反映した第 2 版） |
| ブランチ | `feature/NTKIT-16` |
| 前段階 | 段階 3（C++ API）と段階 4（サンプルの移行）は完了。CU-01 の再実行だけが残っている |
| 反映した決定 | D-4、D-5、D-6（E-5 で見直し）、D-7、D-8（E-17 で例外を明記）、D-9、D-10、E-1〜E-18（4.2） |
| 元にした設計書 | `designs/2026-09-20-windows-architecture-cpp-api-design.md`（C ABI はこの C++ API の上に載る。OP の番号、約束、エラーはそこから引き継ぐ） |
| レビュー | `reviews/2026-09-21-windows-architecture-c-abi-design-review-v1.md`（3 者、30 件） |

- **`PLANNED_SYMBOLS_EXEMPT`**: この設計書は、まだ書かれていない C ABI の名前（`ntk_*`）を定める。実装の前なので、名前が実装に存在するかの照合は T-11 の機械照合が受け持つ

## 0. レビューの反映

3 者（Claude の整合担当、Claude の相互運用担当、Codex）に v1 をレビューしてもらった。30 件の指摘の反映先は次のとおり。R-1、R-5、R-25 は利用者が 2026-09-21 に決めた。
...
- `ntk_clipboard_session_close` が `CANCELED` か `BUSY` を返したら、**メッセージを処理してから**再試行する。処理せずに回し続けると、完了が配送されず終わらない
- 言語ごとの注記:
...
- `AssemblyReloadEvents.beforeAssemblyReload`、`EditorApplication.playModeStateChanged`（`ExitingPlayMode`）、`Application.quitting` で、**オーナーのスレッドから** close を成功するまで呼び、`_free` する
- 閉じ損ねても、`_free` をしていればコールバックは止まる（7.5 の生存フラグ）。`_free` もしないままリロードすると、次の `create` は `NOT_SUPPORTED` になり、回復できない
...
| 通知の活性化 | そのマネージャーの close、`_free`、`set_invoked_handler` |
| すべて | 例外を投げること（7.7） |
...
- C++ API の振る舞いの変更。E-10 は読み手のいない項目の削除、E-13 は C++ のヘッダーが既に約束していることの実装で、どちらも約束を変えない
...
| E-12 | **通知の活性化のハンドラに解放のコールバックを付ける**。配送中の呼び出しがすべて終わった後に、`release(user_data)` をちょうど 1 回呼ぶ。close と差し替えは待たない | 2026-09-21 利用者（R-1） | C++ の配送はロックの外でハンドラを呼ぶので、close や差し替えの後も古いハンドラが走りうる。close の中で待つと、コールバックが UI スレッドへ同期的に戻る作りで行き詰まる |
...
- 失敗しない関数（ハンドルの読み取り、`_free`、`ntk_clipboard_session_can_close`、`ntk_version`、`ntk_last_system_code`）は値をそのまま返す。範囲外の添字や `NULL` のハンドルには `NULL` か 0 を返す。**0 が正しい値でもありうる読み取り（`ntk_notification_list_id_at` など）は、先に件数を確かめる**
- 出力引数:
...
| `ntk_notification_runtime` | **生きているマネージャーが無ければ**、`MddBootstrapShutdown` を呼ぶ。あれば印を付けるだけで、最後のマネージャーが閉じたときに呼ぶ（C ABI で数える） |
...
| 履歴の 3 つのハンドラ | `ntk_clipboard_set_history_handlers` | 差し替えか外す呼び出しが戻った後（オーナーのスレッドで呼ぶので、配送と重ならない）、close が成功した後、または `_free` が戻った後 |
...
| 通知の活性化 | マネージャーの設定、`ntk_notification_manager_set_invoked_handler` | **`release` が呼ばれた後**（E-12）。`release` は、差し替えか close の後、その登録の配送中の呼び出しがすべて戻った時点で、ちょうど 1 回呼ばれる。スレッドは、最後の配送を終えたスレッドか、配送中が無ければ差し替えか close を呼んだスレッド |

#### 7.5.2 生存フラグ（R-2）

セッションの C ABI の包みは、コールバックを C++ API に渡すときに**生存フラグを共有する小さな状態**を捕捉させる。

- すべての C コールバックの呼び出しは、この状態のミューテックスを持ってフラグを確かめてから行う
- `ntk_clipboard_session_free` はミューテックスを持ってフラグを倒す。**`_free` が戻った後、C の関数ポインタは 2 度と呼ばれない**
- 放棄したセッションの C++ 側の資源（ウィンドウ、リスナー、処理中の要求）はプロセスが終わるまで残る。しかし利用者のコードには届かない
- コールバックの中からの `_free` は 1.3.5 で禁じている。ミューテックスは再入を許さない

#### 7.5.3 遅延レンダリング（R-6、R-20）

- provider は形式ごとにコピーされる（C++ の実装）。C ABI の包みは解放ガードを `shared_ptr` で共有し、**最後のコピーが消えたときに `release(user_data)` をちょうど 1 回呼ぶ**
- 手放す時期（C++ の実装どおり）:

| 場面 | `release` |
|---|---|
| 次の `ntk_clipboard_reserve_deferred` が成功した | 前の予約の分を呼ぶ |
| ほかのアプリがクリップボードを消去した（`WM_DESTROYCLIPBOARD`） | 呼ぶ |
| `ntk_clipboard_recover_deferred_state` が成功した | 呼ぶ |
| 予約が失敗した（`PARTIAL_STATE` 以外） | 呼び出しスレッドで、関数が戻る前に呼ぶ |
| **C ABI の入口の検査で拒んだ**（`NULL`、件数 0、不正な UTF-8 など） | **呼び出しスレッドで、関数が戻る前に呼ぶ** |
| 予約が `PARTIAL_STATE` で失敗した | 保持したまま。後の上の場面のどれかで呼ぶ |
| **セッションを close した** | **呼ばない**。次のセッションの予約か、ほかのアプリの消去か、プロセスの終了まで残りうる |
| セッションを放棄した（`_free`） | 呼ばない（7.5.2。provider も 2 度と呼ばれない） |

- `release != NULL` なら、**戻り値にかかわらず**上の規則でちょうど 1 回呼ぶ。利用者はエラーが返ったときに自分で `user_data` を片付けてはならない（履歴の要求と逆なので Doxygen に明記する）
...
- 構造体の規則:
  1. すべての構造体は `uint32_t struct_size; uint32_t reserved0;` で始まる。途中と末尾の詰め物も `reserved` 項目として明示し、**`reserved` が 0 でなければ `INVALID_PARAMETER`**
  2. **最小の大きさは 2.0.0 の `sizeof` に固定**し、それより小さい `struct_size` は `INVALID_PARAMETER`
  3. 項目は `offsetof(項目) + sizeof(項目) <= struct_size` のときだけ読む。足りない項目は既定値（0）とする。**新しい DLL は古い大きさの構造体をそのまま受け付ける**
  4. 知っている大きさを超える部分は、**すべて 0 なら受け付け、0 以外が 1 バイトでもあれば `NOT_SUPPORTED`**（新しいヘッダーで組んだ利用者が、古い DLL に新しい項目の意味を黙って捨てられないようにする。Linux の `copy_struct_from_user` と同じ規則）
...
| OP-09 | `Manager::Close` | `ntk_notification_manager_close(m)`（戻り値 `void`） | 包みの `release` を配送中の件数に結ぶ |
...
| `showFileDialog` | `ntk_dialog_show_open_file` | キャンセルは `*pError == -1` ではなく `NTK_DIALOG_ERROR_CANCELED`。失敗は OS の生の値ではなく `SYSTEM_ERROR` と `ntk_last_system_code()`。フィルターは NUL 区切りの 1 文字列ではなく、名前とパターンの組の配列。`title`、`allow_missing_file`、`owner` が効く（E-13）。**パスの上限は今と同じ 1024 文字**で、超えると `SYSTEM_ERROR` |
| `showMultiFileDialog` | `ntk_dialog_show_open_files` | 先頭のフォルダ名と名前の並びではなく、**フルパスの一覧**。件数にフォルダを数えない（C++ API の OP-03）。上限は合計 32768 文字 |
| `showSaveFileDialog` | `ntk_dialog_show_save_file` | 上書きの確認を `skip_overwrite_prompt` で外せる（今は常に確認）。`title`、`owner` が効く。上限 1024 文字 |
| `showFolderDialog` | `ntk_dialog_show_pick_folder` | キャンセルは `TRUE` + `pError == -1` ではなく `CANCELED` |
| `showMultiFolderDialog` | `ntk_dialog_show_pick_folders` | キャンセルは件数 0 や `-1` ではなく `CANCELED`。成功して 0 件のときとキャンセルが区別できる |
...
| `showNotification` | `ntk_notification_show` | JSON ではなくビルダー。JSON の解析の失敗（`INVALID_PAYLOAD`）は起きなくなり、意味の検証の失敗は今と同じ `INVALID_PARAMETER`。**`timestamp` は Unix 秒から Unix ミリ秒になる**。scenario と音声の名前は、未知の文字列が黙って既定になっていたのが、列挙の範囲外なら `INVALID_PARAMETER` になる |
...
| `copyPlainText` | `ntk_clipboard_copy_text` | **未知のオプションのビットは、捨てられていたのが `INVALID_PARAMETER` になる**。`SENSITIVE`（3）は同じ値で残す |
...
- 音声、進捗、アプリのロゴ以外の構造的な項目（timestamp、expiration）を「無い」に戻す関数は持たない。**戻したいときはビルダーを作り直す**。ビルダーは使い回してよく、`show` はビルダーを変えない
...
| Dialog の内部入口（`WindowsDialogManager` の `Show*Dialog`） | **残す**。C++ API はこの上に実装されている | - |
...
`check_cpp_api_contract.py` は旧 `.def` と旧ヘッダーを読み、無ければ黙って skip する。**T-11 で参照先を新しい `.def` と C ABI のヘッダーに付け替え、読めなければ失敗にする**。
...
| OP-08 | `ntk_notification_manager_create` | 同期 | 呼び出しスレッド | 任意 | 関数ポインタ + `user_data` + `release` → 配送を数える包み |
...
| OP-21 | `ntk_clipboard_session_create` | 同期 | 呼び出しスレッド（オーナーになる） | STA | 関数ポインタ + `user_data` → 生存フラグ付きの包み |
...
- コールバックは、関数ポインタと `user_data` を捕捉した `std::function` に包んで C++ API に渡す。セッションの包みは生存フラグ（7.5.2）を、通知の包みは配送の件数（E-12）を、provider の包みは解放ガード（7.5.3）を共有する
...
| 通知の解除は、登録の取り消しを先に、コールバックの解除を後に | NTF-40 | `Manager::Close` を呼ぶだけ。`release` は配送中の件数が 0 になってから |
...
| **新**: `release` はちょうど 1 回 | 7.5.1、7.5.3 | provider は `shared_ptr` の解放ガード、通知は配送の件数 |
| **新**: `_free` の後、セッションのコールバックは呼ばれない | 7.5.2 | 生存フラグとミューテックス |
...
| CT-04 | 構造体の配置が付録 A どおり | `static_assert(offsetof(...))` と `sizeof` を x64 の値で固定する |
| CT-05 | すべての関数で `NULL` のハンドルと出力引数が `INVALID_PARAMETER` になり、落ちない。ハンドルの出力は失敗でも `NULL` になる | 表で全関数を回す |
| CT-06 | 不正な UTF-8、未知のビット、範囲外の添字と列挙、必須の文字列の `NULL`、履歴のコールバックの `NULL` が `INVALID_PARAMETER`。省略できない素の文字列の `NULL` が `""` | 同上 |
| CT-07 | 構造体の版（7.8）: 小さすぎる、`reserved` が 0 でない、2.0.0 と同じ大きさ、知らない部分が 0 の大きい構造体、知らない部分が 0 でない大きい構造体 | 5 通りを各構造体で |
| CT-08 | `ntk_last_system_code()` が失敗で書かれ、成功で 0 になり、スレッドごとに独立している。完了は値を書かず、引数で渡す | 2 スレッドで交互に失敗させる |
| CT-09 | `_free(NULL)` が何もしない。読み取りの範囲外は `NULL` / 0。**空のバイト列で `size == 0` かつ `data != NULL`**。`out_size` が `NULL` でも読める | 全ハンドルの型で |
...
| CT-11 | 通知の内容のビルダーが C++ の `NotificationContent` と同じペイロードを作る。「無い」と「在るが空」の区別を含む | 同じ内容を C++ と C ABI で作り、`BuildPayload` の結果を比べる。入力一覧 §1.10 の 14 行を網羅する |
...
| CT-13 | provider の `release` がちょうど 1 回。形式が 2 つ以上の予約、次の予約、消去、復旧、入口の検査での拒否、`PARTIAL_STATE`（保持され、後で呼ばれる）、close（呼ばれない）のそれぞれ。`render_target_set` の約束 | 呼び出しを数える |
...
| CT-17 | セッションの `_free`（放棄）の後、変化、履歴のハンドラ、履歴の完了、provider のどれも呼ばれない。`release` も呼ばれない | 生存フラグの検査。放棄の後にイベントを送る |
| CT-18 | 通知の `release`: 配送中に close と差し替えをしても、最後の配送が戻った後にちょうど 1 回呼ばれる。配送中が無ければ close のスレッドで呼ばれる | 活性化を別スレッドで止めておき、競合させる |
| CT-19 | Runtime とマネージャーの順序: マネージャーが生きている間の `runtime_free` は Shutdown を呼ばず、最後の close で呼ぶ。逆の順序でも正しく呼ぶ | Bootstrap の関数を差し替えて数える |
| CT-20 | 履歴の timestamp の変換（WinRT の tick → Unix ミリ秒）。既知の値で | 1970-01-01 と 2026-01-01 の tick |

### 12.2 実際の DLL を通す確認

| ID | 確かめること | 方法 |
|---|---|---|
| CT-21 | 公開ヘッダーと import ライブラリだけで組んだ C の実行ファイルが、実際の DLL で動く | `WindowsLibraryCApiSmoke`（C、`/TC`）が次を行う。`ntk_version` がヘッダーの `NTK_VERSION` と一致する。`ntk_string` などの出力を得て解放する。構造体の入力を渡す。Clipboard のセッションを STA で作り、テキストを往復させ、履歴の可否の完了を 1 回受け取って close する（前面でないときは `NOT_FOREGROUND` の完了で可）。Dialog と通知は UI を出さない関数だけ |
...
| T-02 | `Common.h` と共通の実装（ハンドル、`ntk_last_system_code`、`ntk_version`、UTF-8 の変換と U+FFFD、例外の捕捉、構造体の版の読み方）。CT-01〜CT-04、CT-07〜CT-09 | 1.5日 | T-01 | 挙げたテストが通る | `thread_local` の置き場所。7.8 の規則が共通の関数 1 つにまとまっているか |
| T-03 | C++ の Dialog の修正（E-13）: `owner`、ファイルのダイアログの `title`、`fileMustExist`、`overwritePrompt` を Win32 に渡す。Dialog の UI テストを足す | 1.0日 | 無し | 4 項目が効く。Dialog の UI テストが通る | C++ の既定の振る舞いが変わっていないか |
| T-04 | `Dialog.h` と OP-01〜OP-06。CT-15 | 1.0日 | T-02, T-03 | 6 関数が動く。CT-15 が通る | E-9 の反転。フィルターの組 |
| T-05 | `Notification.h` のランタイム（参照を数える）、マネージャー（配送を数える包みと `release`）、一覧、活性化と、OP-07〜OP-09、OP-12〜OP-20。CT-18、CT-19 | 1.5日 | T-02 | 12 操作が動く。CT-18、CT-19 が通る | `release` のスレッドと回数 |
| T-06 | 通知の内容のビルダーと OP-10、OP-11。CT-11 | 1.5日 | T-05 | CT-11 が入力一覧 §1.10 の 14 行を網羅して通る | `NULL` と `""` の区別 |
| T-07 | Clipboard のセッション（生存フラグ）、履歴のハンドラ、読み書き（OP-21〜OP-34、OP-36〜OP-39）。CT-16、CT-17 | 1.5日 | T-02 | 挙げた操作が動く。CT-16、CT-17 が通る | 生存フラグとミューテックスの範囲 |
| T-08 | 複数形式のビルダー（OP-35）と遅延レンダリング（OP-40、OP-41）。CT-10、CT-13 | 1.0日 | T-07 | CT-10、CT-13 が通る | 解放ガードの共有 |
| T-09 | 履歴の要求（OP-42〜OP-47）と履歴のハンドル。CT-12、CT-20 | 1.5日 | T-07 | CT-12、CT-20 が通る | 完了のスレッド、回数、システムコード |
| T-10 | `WindowsLibraryCApiSmoke`（CT-21） | 0.5日 | T-04, T-06, T-08, T-09 | 実際の DLL で通る | 公開ヘッダーと import ライブラリだけで組んでいるか |
| T-11 | 機械照合（12.3）を `scripts/` に足し、壊すと落ちることを確かめる。`check_cpp_api_contract.py` の参照先を付け替える | 1.0日 | T-04, T-06, T-08, T-09 | 照合が通る。壊した入力で落ちる。ファイルが無ければ失敗する | 両辺をソースから導出しているか |
| T-12 | 今の C ABI を削除する。定数と typedef を `*Codes.h` へ移し、旧ヘッダー 4 つ、`src/Bridge/`、旧 DLL、旧 C ABI のテスト 3 群を消す。`CommonInternal.h` の `extern "C"` を外す。E-10 の整理 | 1.5日 | T-11 | 単体テストが通る。Core に `extern "C"` の関数宣言が無い | 移した定数の値が変わっていないか。消した項目の参照が残っていないか |
...
| T-14 | `windows.md` の修正（5.2）。公開ヘッダーの Doxygen（スレッド、寿命、`user_data` の期間、借りたポインタ、エラー、`release` の規則）。C++ の公開ヘッダーの Doxygen のうち旧 C ABI に触れるものを全件検索して直す | 1.0日 | T-12 | 5.2 の 6 か所が直っている。全公開宣言にコメントがある。C++ の Doxygen に旧 C ABI の記述が無い | 1.3 と 7.3〜7.5 がヘッダーに書かれているか |
| T-15 | 8.3 の対応表をヘッダーと突き合わせ、`unity-native-plugin` への申し送り（1.3.4 の Unity の手順を含む）を結果文書に書く | 0.5日 | T-13 | 52 関数すべてに行がある | 振る舞いの差が漏れていないか |
...
- 先行: T-01、T-02、T-03（T-03 は T-01 と並行できる）
...
- [ ] E-10 の整理と E-13 の Dialog の修正が済んでいる

### 15.2 品質

- [ ] CT-01〜CT-21 が通る
- [ ] C++ API の単体テストと `scripts/test_windows.ps1`（全件）が通る。Dialog の UI テストを足した
...
#define NTK_SYSTEM_CODE_E_OUTOFMEMORY 0x8007000Eu
