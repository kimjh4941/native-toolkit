...
| E-11 | フォルダ `windows/WindowsLibrary/` は **`WindowsLibraryCore/` に改名しない**（v3 までは段階 6 に回すとしていた） | 2026-09-21 利用者 | フォルダは `WindowsLibrary.sln` とビルドの出力先も兼ねており、改名すると sln、サンプル、テスト、`.props` の import、スクリプト、規約、各機能の文書の参照を付け替えることになる。得られるのは名前の一致だけで、プロジェクト、`.lib`、`.props` の名前が `WindowsLibraryCore` なので中身の区別は付く。**2026-09-23 追記**: 利用者の決定でプロジェクトと成果物を `WindowsLibrary` に戻した（段階 6 の結果 5 章）。フォルダーを改名しないという決定はそのままで、`Core` を付ける理由（1.x の `WindowsLibrary.vcxproj` との衝突回避）が T-12 で消えたため |
