...
- **FlaUI で自動化する。** FlaUI で扱えない項目（画像の中身、バッジのグリフの種類）だけを computer use で確かめる（README 7.2、7.3）
...
| `IBanner` | タイトルでバナーを探す（`INotificationItem` と同じ形）、消えるのを待つ |
| `ITaskbarBadge` | バッジの状態を読む（無し / 数字 / グリフ） |
...
| D-06 | 複数ファイル → 2 つ選ぶ | `Result: 3`、`[0]` がフォルダ、`[1]` `[2]` がファイル名（付録 A の注記） |
...
| N-14 | SetBadgeGlyph → ClearBadge | バッジがグリフ（名前が空）→ 無し（グリフの種類は CU-02） |
...
| CU-02 | バッジのグリフ | SetBadgeGlyph | タスクバーのサンプルアプリのアイコンに alert のグリフが表示されている |
