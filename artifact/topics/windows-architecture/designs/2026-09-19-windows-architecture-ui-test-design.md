...
| D-14 | 保存 → 既存のファイルを選ぶ | 上書きの確認が出て、はいで `Result: 1`、パスが返る。段階 5 の E-13 で追加（`overwritePrompt` を C++ の実装が Win32 に渡すようになったため）。あわせて D-03、D-05、D-07 でファイルのダイアログのタイトル（`Open File` / `Open Files` / `Save File`）を確かめる |
