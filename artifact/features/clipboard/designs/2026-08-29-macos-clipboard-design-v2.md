...
| Manager は callback 版 + `async throws` 版 | 適合 | 全ての待機操作で併設 |
...
| `.../Notification/Domain/Error/BridgeError.swift` | **変更なし**。Clipboard の Bridge 境界でも再利用する | なし |
...
│       ├── ClipboardPasteResult.swift              部分失敗を持つ（H-8）
...
```swift
public struct ClipboardDetectedLink: Sendable, Equatable { public let url: String }
...
public struct ClipboardPasteResult: Sendable, Equatable {
    public let items: [ClipboardItemData]      // 成功した分だけ
    public let failures: [ClipboardError]      // 失敗した provider ごと
    public let isPartial: Bool                 // failures が空でなければ true
}
...
- View が破棄されたら進行中の `Progress` を `cancel()` し、`onPaste` は呼ばない
...
```
active ──(writePromiseTo 開始)──> fulfilling ──(completionHandler)──> active
  │                                    │
  │                                    └─(release 要求)─> pendingRelease ─(履行完了)─> released
  └─(release 要求 / changeCount 不一致で stale 検出)─> released
```

- `releaseFilePromise(handle:)` は**冪等**。既に `released` なら何もしない
- `fulfilling` 中の release 要求は `pendingRelease` に遷移し、履行完了後に解放する（履行中の強制解放をしない）
- stale 判定は `ownership.changeCount` と現在値の不一致
- ハンドルが未登録なら `promiseHandleNotFound`
...
|---|---|---|---|
| OP-01 | copy | `copy(_:options:scope:completion:)` | `copy(_:options:scope:) throws -> PasteboardOwnership` |
| OP-02 | append | `append(_:ownership:completion:)` | `append(_:ownership:) throws -> PasteboardOwnership` |
| OP-03 | read | `read(scope:completion:)` | `read(scope:) throws -> ClipboardReadResult` |
| OP-04 | readData | `readData(utType:scope:completion:)` | `readData(utType:scope:) throws -> Data?` |
| OP-05 | snapshot | `snapshot(matchingTypes:scope:completion:)` | `snapshot(matchingTypes:scope:) throws -> ClipboardSnapshot` |
| OP-06 | clear | `clear(scope:completion:)` | `clear(scope:) throws -> Int` |
| OP-07 | createPasteboard | `createPasteboard(_:completion:)` | `createPasteboard(_:) throws -> PasteboardScope` |
| OP-08 | removePasteboard | `removePasteboard(_:completion:)` | `removePasteboard(_:) throws` |
...
**同期ネイティブ版の理由**: OP-01〜OP-08、OP-12〜OP-17 のシステム API は同期。common.md の「システム API に合わせた同期・非同期設計」に従い、`async` を付けない。`@MainActor` 隔離のため、他アクターからの呼び出しは `await` が必要になる。
...
### 8.4 Unity Bridge 契約（H-2 / 新規設計判断）
...
| callback 回数 | **各呼び出しにつき exactly-once**。JSON パース失敗など早期リターン経路でも必ず 1 回呼ぶ |
| callback が NULL | 呼び出しは実行するが結果を通知しない。エラーにしない |
| 引数が NULL | `BridgeError.contractViolation`（1302）を callback で返す |
| JSON パース失敗 | `BridgeError.parseFailed`（1301）を callback で返す |
...
/// JSON 文字列を返す操作の結果。json は isSuccess=YES のときのみ非 NULL。
typedef void (*ClipboardJsonCallback)(BOOL isSuccess,
                                      const char* json,
                                      NSInteger errorCode,
                                      const char* errorMessage);

/// 変更監視のイベント通知。購読中は複数回呼ばれる。
typedef void (*ClipboardChangeCallback)(const char* scopeJson,
                                        NSInteger changeCount);

/// File Promise 受領のイベント通知。終端で isFinished=YES が 1 回だけ来る。
...
| C 関数 | 引数 | callback | 対応 OP |
|---|---|---|---|
| `clipboardCopy` | `contentJson`, `optionsJson` | `ClipboardJsonCallback`（ownership を返す） | OP-01 |
| `clipboardAppend` | `contentJson`, `ownershipJson` | `ClipboardJsonCallback` | OP-02 |
| `clipboardRead` | `scopeJson` | `ClipboardJsonCallback` | OP-03 |
| `clipboardReadData` | `utType`, `scopeJson` | `ClipboardJsonCallback`（Base64） | OP-04 |
| `clipboardSnapshot` | `matchingTypesJson`, `scopeJson` | `ClipboardJsonCallback` | OP-05 |
| `clipboardClear` | `scopeJson` | `ClipboardJsonCallback`（changeCount） | OP-06 |
| `clipboardCreatePasteboard` | `requestJson` | `ClipboardJsonCallback`（scope） | OP-07 |
| `clipboardRemovePasteboard` | `scopeJson` | `ClipboardCallback` | OP-08 |
| `clipboardDetectPatterns` | `patternsJson`, `scopeJson` | `ClipboardJsonCallback` | OP-09 |
| `clipboardDetectValues` | `patternsJson`, `scopeJson` | `ClipboardJsonCallback` | OP-10 |
| `clipboardDetectMetadata` | `scopeJson` | `ClipboardJsonCallback` | OP-11 |
| `clipboardAccessBehavior` | `scopeJson` | `ClipboardJsonCallback` | OP-12 |
| `clipboardStartObserving` | `scopeJson`, `intervalSeconds` | `ClipboardCallback` + `ClipboardChangeCallback` | OP-13 |
| `clipboardStopObserving` | なし | `ClipboardCallback` | OP-14 |
| `clipboardCheckForegroundChange` | `scopeJson` | `ClipboardJsonCallback`（bool） | OP-15 |
| `clipboardProvideFilePromise` | `requestJson` | `ClipboardJsonCallback`（handle） | OP-16 |
| `clipboardReleaseFilePromise` | `handleJson` | `ClipboardCallback` | OP-17 |
| `clipboardReceiveFilePromises` | `destinationPath`, `scopeJson`, `policyJson` | `ClipboardReceiptCallback` | OP-18 |

**OP-19（makePasteButton）は `NSView` を返すため Bridge に公開しない。**

**File Promise の `write` クロージャは C ABI に載らないため、Bridge 版 OP-16 は「あらかじめ用意されたファイルパスの内容を書き出す」形に限定する**（`requestJson` に `sourcePath` を持たせ、Swift facade がクロージャを合成する）。
...
// contentJson: representations は UTI -> Base64
...
// 成功レスポンス（clipboardRead）
{ "changeCount": 12,
  "items": [ { "representations": { "public.utf8-plain-text": "aGVsbG8=" } } ] }
```

`Data` は必ず **Base64** で受け渡す。数値は JSON number、`changeCount` は 64bit 整数。
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| OP-01 | C-02 + C-03（同期） | 同期 throws | 同期 throws | `Task{@MainActor}` | 同期 throws | callback | MainActor | なし / `PasteboardOwnership` を返す | なし | callback 1 回 | 該当なし | Bridge は C ABI と actor 隔離のため callback（§6.6） |
| OP-02 | C-03（同期） | 同期 throws | 同期 throws | 同上 | 同期 throws | callback | MainActor | なし / 所有権照合 | なし | callback 1 回 | 該当なし | 同上 |
| OP-03 | R-07 + R-09（同期） | 同期 throws | 同期 throws | 同上 | 同期 throws | callback | MainActor | なし / 値型返却 | なし | callback 1 回 | 該当なし | 同上 |
| OP-04 | R-05（同期） | 同期 throws | 同期 throws | 同上 | 同期 throws | callback | MainActor | なし | なし | callback 1 回 | 該当なし | 同上 |
| OP-05 | Q-01 / Q-05（同期） | 同期 throws | 同期 throws | 同上 | 同期 throws | callback | MainActor | なし | なし | callback 1 回 | 該当なし | 同上 |
| OP-06 | C-01（同期） | 同期 throws | 同期 throws | 同上 | 同期 throws | callback | MainActor | なし | なし | callback 1 回 | 該当なし | 同上 |
| OP-07 | P-02 / P-03（同期） | 同期 throws | 同期 throws | 同上 | 同期 throws | callback | MainActor | 一意名は release 必須 | なし | callback 1 回 | 該当なし | 同上 |
| OP-08 | P-04（同期 oneway） | 同期 throws | 同期 throws | 同上 | 同期 throws | callback | MainActor | 標準名は拒否 | なし | callback 1 回 | 該当なし | 同上 |
...
| OP-19 | U-02（callback）+ `NSItemProvider` load（async） | - | - | なし | 同期 throws（factory）。`onPaste` は非同期後に 1 回 | 非公開 | MainActor | View 破棄で `Progress.cancel()` | **既定 15 秒** | **`onPaste` は exactly-once** | timeout 後の完了は破棄 | factory は同期。payload 変換のみ非同期（§7.11） |
...
Bridge 境界の失敗（JSON パース、NULL 引数）は **`BridgeError`（1301 / 1302）** を使い、`ClipboardError` には含めない（H-2）。
...
| 18 | `filePromiseWriteFailed(String)` | 書き出しクロージャが throw | Data |
| 19 | `filePromiseReceiveFailed(String)` | reader が error を返した | Data |
| 20 | `filePromiseTimedOut(seconds: Int)` | `overallTimeout` 到達 | Data |
| 21 | `promiseHandleNotFound` | 未登録ハンドルの操作 | Manager（coordinator） |
| 22 | `destinationNotWritable(String)` | 受領先が不正 | Application |
| 23 | `pasteLoadFailed(String)` | `NSItemProvider` のロード失敗 | Presentation |
| 24 | `pasteLoadTimedOut(seconds: Int)` | PasteButton のロード timeout | Presentation |
| 25 | `cancelled` | Task / 明示キャンセル | 全層 |
| 26 | `unknown(String)` | 上記以外 | 全層 |

**v1 から削除**: `noMatchingItem`（M-1）、`imageDecodeFailed` / `imageEncodeFailed`（L-1）、`observationAlreadyActive`（M-5）。
...
| 1512 | `emptyTypeFilter` | `The type filter must not be empty. Pass nil to disable filtering.` |
...
| 1520 | `filePromiseTimedOut` | `Receiving promised files timed out after {seconds} seconds.` |
| 1521 | `promiseHandleNotFound` | `The promise handle is not registered.` |
| 1522 | `destinationNotWritable` | `The destination directory is not writable: {path}.` |
| 1523 | `pasteLoadFailed` | `Failed to load pasted item: {reason}.` |
| 1524 | `pasteLoadTimedOut` | `Loading pasted items timed out after {seconds} seconds.` |
| 1525 | `cancelled` | `The clipboard operation was cancelled.` |
...
| `ReleaseFilePromiseUseCase` | 解放 | `promiseHandleNotFound` | **二重 release が冪等**（M-4） |
...
| `ClipboardError` | 全 26 ケースの code / message | - | コード重複なし。`BridgeError` と衝突なし |
...
|---|---|---|---|---|---|
| T-01 | Domain モデル + `ClipboardLimits` + `ClipboardError`（26 ケース / 1501〜1599） | 1.0日 | なし | `ClipboardErrorTests` 全通過 | Domain に platform 型なし。コード重複なし。`BridgeError` と衝突なし |
...
| T-08 | `MacClipboardManager` 骨格（DI、callback + native、`Log.d`） | 1.0日 | T-07 | 全 OP が呼べ、callback が MainActor | mac.md の併設。CT-04 |
| T-09 | 検出 API + `ClipboardDetectionMapper`（**DDMatch\* 全フィールド**）+ バージョン分岐 | 1.5日 | T-08 | IT-20、Mapper テスト通過 | **M-7 の非可逆変換がないこと**。**15.4 未満に Q 群フォールバックを置かないこと（RK-01）** |
...
| T-11 | File Promise 提供側（delegate / registry / lifecycle） | 1.5日 | T-07, T-08 | IT-12〜IT-14、CT-03、CT-06 通過 | **RK-21: coordinator が provider+delegate を強参照。completion は要求ごとに 1 回。全体で 1 回に絞るガードを置かない** |
| T-12 | **File Promise 受領側（静穏 + 全体タイムアウトの終了モデル）** | 1.5日 | T-11 | IT-15〜IT-19 通過 | **H-3: `fileTypes.count` を終了判定に使っていないこと。`finished` が exactly-once。終端後の late callback 破棄** |
| T-13 | OP-18 の native async（`AsyncThrowingStream` + `async throws`） | 0.5日 | T-12 | ストリームが終端し、Task キャンセルで `.cancelled` になる | **H-4: common.md の併設要求** |
| T-14 | `ClipboardPasteLoader` + `PasteButtonFactory` | 1.0日 | T-08 | PT-01〜PT-05 通過 | **H-8: 部分失敗・timeout・exactly-once・cancel** |
| T-15 | `LazyDataProvider`（内部限定） | 0.5日 | T-07 | 大容量 copy が遅延提供を通る | RK-17。所有は coordinator |
| T-16 | Unity Bridge（facade / parser / `.h` / `.m`）+ §8.4 の契約実装 | 2.0日 | T-08〜T-14 | BT-01〜BT-09 通過 | **H-2: 全 endpoint の exactly-once / NULL / parse failure**。**H-7: nonisolated facade + 任意スレッド可**。ObjC 型規約 |
...
- [x] レビュー指摘 20 件（高 8 / 中 11 / 低 1）をすべて反映した。反映先は §0 の対応表
...
- [x] ドメインエラー 26 ケースを §10 に列挙し、`BridgeError` と責務分離した
...
- [ ] 12.3 の Presentation テスト PT-01〜PT-05 が全通過する
- [ ] 12.4 の Bridge テスト BT-01〜BT-09 が全通過する
- [ ] 12.5 の並行性テスト CT-01〜CT-07 が全通過する
