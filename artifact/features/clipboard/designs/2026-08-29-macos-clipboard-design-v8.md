# macOS クリップボード機能 実装設計書 v8

- 作成日: 2026-08-29
- 改訂日: 2026-09-02（v8: MT-05 と T-18 完了条件を、範囲外の D&D に依存しない検証手順へ差し替え）
- 前版: `artifact/features/clipboard/designs/2026-08-29-macos-clipboard-design-v7.md`
...
| MT-05 | **OP-16 で File Promise を提供 → Finder に貼り付け → 約束したファイルが生成される**（§12.6.1） | 同上 |
...
#### 12.6.1 MT-05 が貼り付け経路である理由（v8 で変更）

v7 までの MT-05 は「Finder へドラッグしてファイル生成」であり、併せて T-18 の完了条件に
「MT-05 用の最小 drag harness」を課していた。**これは本設計自身のスコープ境界と矛盾する。**

- ドラッグ&ドロップは企画書（`research-v3.md`）で明示的に調査対象外としている
- iOS 設計 v4 も D&D を Out of scope に置いており、Android 設計は言及すらない
- つまり 3 プラットフォームが揃って範囲外にした操作を、macOS だけが出荷機能の必須検証手段に
  指定していた

さらに、ドラッグ経路は**製品の利用者が通る道ではない**。

| 経路 | Unity から呼べるか |
|---|---|
| ペーストボード（OP-16 → 貼り付け） | 可。`clipboardProvideFilePromise` が Bridge にある |
| ドラッグ | 不可。`NSView` を扱うため Bridge 非公開（OP-19 と同じ理由） |

ドラッグを検証しても、実際の呼び出し元が通る経路は一度も通らない。**検証すべきは出荷する
経路**であるため、MT-05 を貼り付け経路へ差し替える。

**未確認事項**: Finder が Cmd-V で `NSFilePromiseProvider` の約束を履行するかは未検証である。
`NSFilePromiseProvider` は `NSPasteboardWriting` に準拠するため一般ペーストボードには書けるが、
Apple の文書は D&D の文脈で書かれており、貼り付け経路の保証は読み取れない。

- **履行される場合**: 本表の手順で MT-05 を実施する
- **履行されない場合**: MT-05 を「実施不能」として記録し、その理由（提供側の実機検証手段が
  範囲内に存在しない）を本節へ追記する。範囲外の D&D へ踏み込む API 追加で埋めることはしない

いずれの場合も、OP-16 の自動テスト（IT-13 / IT-14 系および CT 群）は据え置きであり、
提供側の実装そのものは検証済みのまま変わらない。
...
| T-18 | サンプルアプリ対応 | - | T-17 | **`design-sample-app` で設計。完了条件は「全公開 OP が `MacLibraryExample` から Unity 非依存で実行できること」および「MT-05 を §12.6.1 の貼り付け経路で実施できること」** | common.md のサンプル依存方向 |
