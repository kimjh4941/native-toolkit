# macOS クリップボード機能 実装設計書 v6

- 作成日: 2026-08-29
- 改訂日: 2026-08-29（v6: 第 5 回レビュー指摘 13 件を全件反映）
- 前版: `artifact/features/clipboard/designs/2026-08-29-macos-clipboard-design-v5.md`
- 初版: `artifact/features/clipboard/designs/2026-08-29-macos-clipboard-design.md`
- レビュー: `.../2026-08-29-macos-clipboard-design-review-v5.md`（第 5 回）、`-v4.md`、`-v3.md`、`-v2.md`、`.../2026-08-29-macos-clipboard-design-review.md`（第 1 回）

> **正規契約の所在（v6 で確立）**
> File Promise 提供側の契約は **§7.12 を唯一の正規**とする。§8.3 の Port、§9 の対応表、§8.4 の Bridge、テスト、DoD は §7.12 から導出したものであり、矛盾した場合は §7.12 が優先する。重複していた旧 transaction 記述は v6 で削除した。
...
- **即時 control / factory に該当する native API（OP-12〜OP-15、OP-17、OP-19、OP-20）のみ同期**。**OP-16 は `.snapshot` の I/O を伴うため `async throws`**（§7.12 が正規。R4-H3 / R5-H2）（common.md の control / factory 例外）。`@MainActor` 隔離のため、他アクターからの呼び出し側は `await` する。**OP-01〜OP-11 の native は `async throws`**（§8.1。R3-M7）
...
[snapshot Q] 2. if case .snapshot(let src) = request.source {
                    ...
                    try Task.checkCancellation()          // late completion を検出
                }
                // 失敗 / キャンセル時は部分・完成 staging を削除して throw。
                // 登録は行わない（R5-M6）
...
- **`onTermination` は資源解放専用**。receipt session の解除、静穏 / 全体タイマの停止、reader / event クロージャの解除を行い、イベント配送は試みない。**提供側の staging は対象外**（R4-L9 / R5-L12）
...
| API | 用途 | terminal event |
|---|---|---|
| `cancelReceipt(_:)` | 公開 OP-20。購読中なら `.finished(.cancelled)` を配送 | **配送する** |
| `discardReceiptAfterStartFailure(_:)` | 開始失敗の内部 rollback 専用 | **配送しない**。session / timer / reader / event closure を解除するだけ |
...
| OP-16 | F-01 + C-03（同期） | **async throws** | **async throws** | なし | **async throws** | callback | MainActor（登録）/ nonisolated（書き出し） | **coordinator が provider+delegate を強参照。stale / 明示 release で解放** | なし | 登録 callback 1 回。**writePromiseTo は要求ごとに completion 1 回** | 該当なし | **`.snapshot` の I/O を伴うため即時 factory 例外に該当せず、callback + native async を併設（R4-H3 / R5-H2）** |
...
| **IT-45** | **snapshot コピー完了後に Task がキャンセルされた場合、完成 staging が削除され登録されない**（late completion） | **R5-M6** |
...
| T-11a | **`FilePromiseLifecycleState`**（lock-owning、`CommitReleaseOutcome`、`activate`、generation claim） | 1.0日 | T-07 | CT-08〜CT-12、IT-41 通過 | **R4-H2 / R5-H1 / R5-H3: `@MainActor` state を触っていないこと。`.released` で必ず辞書除去** |
| T-11b | **`FilePromiseSnapshotter`**（専用 serial queue、cancellation、late completion cleanup） | 1.0日 | T-07 | IT-34、IT-39、IT-45 通過 | **R4-H3 / R5-M6: MainActor をブロックしない。キャンセル時に部分・完成 staging を削除** |
| T-11c | File Promise 提供側の統合（delegate / registry / 正規 transaction / stale tick） | 1.5日 | T-11a, T-11b, T-08 | IT-12〜IT-14、**IT-21〜IT-24、IT-31〜IT-33、IT-40、IT-42、IT-44**、CT-03、CT-06 通過 | **R5-H2: §7.12 の正規 transaction と一致。`request.source` を switch する delegate 契約**。**RK-21: coordinator が provider+delegate を強参照。completion は要求ごとに 1 回。全体で 1 回に絞るガードを置かない**。**R2-M5: release は完全冪等・非 throwing。R2-M12: register→write→rollback** |
| T-12 | **File Promise 受領側（静穏 + 全体タイムアウト + OP-20 cancel + `CancelReceiveFilePromisesUseCase` + silent rollback）** | 2.0日 | T-11c | IT-15〜IT-19、**IT-25〜IT-27、IT-30、IT-36〜IT-38、IT-43** 通過 | **H-3: `fileTypes.count` を終了判定に使っていないこと。`finished` が exactly-once。終端後の late callback 破棄**。**R2-H2: 公開 cancel。R2-M6: §7.12.3 の truth table どおり throw しないこと** |
| T-13 | OP-18 の native async（**`throws -> FilePromiseEventSubscription`** + 集約 async + `ReceiptCompletionGate`） | 1.0日 | T-12 | IT-27、IT-30、**IT-35**、**CT-13〜CT-16** 通過 | **H-4: common.md の併設要求**。**R3-H1: factory が `throws`、要素型は非 throwing、`onTermination` は資源解放専用で配送しない**。**R2-M6: 集約版の通常終端は throw せず partial receipt を返す。Task キャンセル時のみ `CancellationError`** |
...
| T-16a | Unity Bridge Swift facade + `UnityMacClipboardJsonParser`（**実体 20 JSON 型**の入出力） | 1.5日 | T-08〜T-14 | BT-06、BT-07、BT-11 通過 | **H-7: nonisolated facade + 任意スレッド可**。**R2-M11: staging は Manager 側。`sourcePath` をログに出さない** |
| T-16b | Unity Bridge C 層（`.h` / `.m`、**19 endpoint**、callback 契約） | 1.0日 | T-16a | BT-01〜BT-05、BT-08〜BT-10、BT-12〜BT-19 通過 | **R2-H3: prototype が §8.4.3 と一致。operation は exactly-once、event は N 回**。**R3-M4 / R4-M6 / R5-M8: operation callback 必須 3 件、event callback 必須 2 件**。**R5-H5: OP-16 に `scopeJson`**。ObjC 型規約（`BOOL` / `NSInteger`） || **T-16c** | **既存 `BridgeError.swift` の DocC 修正（R2-M14）** | 0.25日 | T-16b | DocC が NULL callback 許容と矛盾しない | **enum ケース・code・message を変更しないこと** |
...
合計見積: 約 22.5 日（T-18 を除く。全 20 タスク、各 0.25〜2.0 日）
...
- [x] **T-11 を 3 分割し、全タスクを 0.5〜2.0 日粒度に収めた（R5-L13）**
