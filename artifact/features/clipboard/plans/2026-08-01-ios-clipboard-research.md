...
- 作成日: 2026-08-01
- 改訂日: 2026-08-01（初版に第 1 回レビュー指摘を反映。以降の改訂は `-v2` 以降の別ファイル）
- 最新版: `artifact/features/clipboard/plans/2026-08-01-ios-clipboard-research-v2.md`（第 2 回レビュー反映済み。以降はこちらを参照する）
- 対象OS: iOS 18 以降
...
| API | 目的 | 主要引数 | 返却値 | エラーケース | 最小条件 |
|---|---|---|---|---|---|
| `registeredTypeIdentifiers` | 登録された UTI 一覧（登録順） | なし | `[String]` | 空配列 | iOS 11.0 |
...
| `hasItemConformingToTypeIdentifier(_:)` | 指定 UTI に適合するデータ表現の有無 | `String` | `Bool` | なし | iOS 11.0 |
...
| `loadItem(forTypeIdentifier:options:completionHandler:)` | 型強制ポリシーに従って item をロード | `String`, `[AnyHashable: Any]?`, `CompletionHandler?` | Void | 型強制失敗 | iOS 8.0 |
| `loadTransferable(type:completionHandler:)` | `Transferable` としてロード | `T.Type`, `(Result<T, Error>) -> Void` | `Progress` | `.failure` | iOS 16.0 |
| `Progress.cancel()` | 進行中のロードをキャンセル | なし | Void | キャンセル後は completion に `NSUserCancelledError` 相当が返る（要検証） | iOS 7.0 |
| `NSItemProvider.errorDomain` / `NSItemProvider.ErrorCode` | エラー判定用のドメインとコード（`unknownError` / `itemUnavailableError` / `unexpectedValueClassError` / `unavailableCoercionError`） | - | `String` / `enum` | - | iOS 8.0 |
...
| `contains(pasteboardTypes:)` | 指定 UTI の保持有無 | `[String]` | `Bool` | なし | iOS 3.0 |
| `contains(pasteboardTypes:inItemSet:)` | 指定アイテム群での保持有無 | `[String]`, `IndexSet?` | `Bool` | なし | iOS 3.0 |
...
| API | 目的 | 主要引数 | 返却値 | エラーケース | 最小条件 |
|---|---|---|---|---|---|
| `pasteboard.changeCount` | 内容変更回数（ポーリング判定用） | なし | `Int` | なし。**アラートを出さない** | iOS 3.0 |
...
| 変更通知の配信条件 | `changedNotification` は他アプリによる変更やバックグラウンド中に配信されない可能性が高い | フォアグラウンド復帰時に `changeCount` を比較するポーリング方式を併用。`changeCount` の読み取りはアラート対象外 |
...
| `isPersistent` の非推奨 | iOS 10 以降、名前付きペーストボードは非永続。`isPersistent` / `setPersistent(_:)` は非推奨 | 使用しない。永続共有が必要なら App Group + 名前付きペーストボード、または別の共有手段を検討 |
...
| 3 | 他アプリでコピーされた内容 | `hasStrings` / `contains(pasteboardTypes:)` のみ | 通知・アラートとも出ない | 未計測 |
| 4 | 他アプリでコピーされた内容 | `changeCount` のみ読む | 出ない | 未計測 |
...
final class ItemProviderLoader {
    ...
    // completion は必ずメインスレッドで返す（loadObject の完了スレッドは保証されない）
    func loadFirstText(completion: @escaping (Result<String, Error>) -> Void) {
        guard let provider = UIPasteboard.general.itemProviders.first(
            where: { $0.canLoadObject(ofClass: NSString.self) }
        ) else {
            completion(.failure(NSError(domain: NSItemProvider.errorDomain,
                                        code: NSItemProvider.ErrorCode.itemUnavailableError.rawValue)))
            return
        }
        let progress = provider.loadObject(ofClass: NSString.self) { object, error in
            DispatchQueue.main.async {
                ...
                completion(.success(text as String))
            }
        }
        progresses.append(progress)
    }

    // 一時ファイル URL は completion を抜けると削除されるため、ブロック内でコピーする
    func loadFirstImageFile(completion: @escaping (URL?) -> Void) {
        guard let provider = UIPasteboard.general.itemProviders.first(
        ...
        let progress = provider.loadFileRepresentation(
            ...
            let dest = FileManager.default.temporaryDirectory
                .appendingPathComponent(provider.suggestedName ?? UUID().uuidString)
    ...
    func cancelAll() {
        progresses.forEach { $0.cancel() }  // キャンセルは正常系として扱う
...
func clipboardSummary() -> String {
    ...
    var kinds: [String] = []
    if pb.hasStrings { kinds.append("string") }   // 本体を読まないためアラートなし
...
final class ClipboardWatcher {
    ...
    // フォアグラウンド復帰時に changeCount を比較する（読み取りではないのでアラートは出ない）
    ...
    override func paste(itemProviders: [NSItemProvider]) {
        for provider in itemProviders where provider.canLoadObject(ofClass: NSString.self) {
            provider.loadObject(ofClass: NSString.self) { object, _ in
                ...
                DispatchQueue.main.async { print("pasted: \(text)") }
            }
        }
    }
}
...
| 項目 | 内容 |
|---|---|
| 許可アラート / アクセス通知の発生条件 | 上記「プライバシー挙動のテスト行列」11 パターンを iOS 18 実機で実測し、iOS 14 のアクセス通知（読み取り後バナー）と iOS 16 の許可アラート（読み取り前ダイアログ）のどちらが出るかを区別して記録する |
...
- [ ] `loadObject` / `loadDataRepresentation` でテキスト・画像を非同期取得でき、結果がメインスレッドで返る
- [ ] `loadFileRepresentation` の一時 URL を completion 内でコピーし、抜けた後も利用できる
- [ ] ロード中に `Progress.cancel()` でキャンセルでき、キャンセル時のエラーを正常系として処理している
- [ ] ロード失敗・型不一致時に `NSItemProvider.errorDomain` / `ErrorCode` でエラー種別を判別できる
- [ ] 画面破棄時に未完了ロードがキャンセルされ、コールバックが解放後に呼ばれない
...
- [ ] 「プライバシー挙動のテスト行列」11 パターンを iOS 18 実機で実測し、iOS 14 アクセス通知と iOS 16 許可アラートを区別して結果を記録した（出る／出ないの断定はせず観測値を記載）
- [ ] `hasStrings` / `hasURLs` / `hasImages` / `hasColors` / `contains(pasteboardTypes:)` / `changeCount` のみの使用では通知・アラートが観測されないことを記録した
