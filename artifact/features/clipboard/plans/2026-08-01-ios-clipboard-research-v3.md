...
- 作成日: 2026-08-01
- 改訂日: 2026-08-01（v3: 第 3 回レビュー指摘を反映）
- 前版: `artifact/features/clipboard/plans/2026-08-01-ios-clipboard-research-v2.md`
- 対象OS: iOS 18 以降
...
| `pasteboard.itemProviders` (get) | `NSItemProvider` 配列取得（非同期ロード可能） | なし | `[NSItemProvider]` | なし。**通知・許可プロンプトの契機（getter 取得時点かロード実行時か）は要検証**（テスト行列ケース 13・14） | iOS 11.0 |
...
| 事前判定 API の通知非対象は一部のみ公式保証 | 公式が非対象として列挙するのは `numberOfItems` / `types` / `itemSet` / `has*` / パターン検出のみ。`contains(pasteboardTypes:)` と `changeCount` は列挙外 | 列挙済み API を第一選択とする。列挙外の 2 つはテスト行列（`changeCount` はケース 4、`contains` はケース 12）で個別観測し、結果と OS バージョンを記録してから公開 API に採用する |
| `UIPasteControl` の保証範囲 | 公式が保証するのは iOS 16 型の許可プロンプトを出さないこと。iOS 14 型のアクセス通知まで出ないとは明記されていない | 「許可プロンプトを回避する経路」と表現し、アクセス通知はテスト行列ケース 7・15 で別列として観測する |
...
| 3 | 他アプリでコピーされた内容 | `hasStrings` / `numberOfItems` / `types` / `itemSet` のみ（公式が非対象と列挙する API） | 出ない（公式列挙） | 出ない（公式列挙） | 未計測 | 未計測 |
...
| 14 | ケース 13 の直後 | 取得済み provider に対し `loadObject` / `loadDataRepresentation` を実行 | ロード時が契機かを切り分ける | 同左 | 未計測 | 未計測 |
| 15 | 他アプリでコピーされた内容 | `UIPasteControl` タップ → `paste(itemProviders:)` 内でロード | 経路全体で出ないか | **プロンプトなしでも通知が出る可能性を要観測** | 未計測 | 未計測 |

ケース 12〜15 は、事前判定 API の安全性（12）と、`itemProviders` の取得契機とロード契機の切り分け（13〜15）を目的とする。ケース 7・15 は「許可プロンプト回避＝アクセス通知も回避」ではない可能性を確認する。
...
1. **completion は必ず 1 回だけ、メインスレッドから返す**。対象は成功・provider エラー・型不一致・provider 不在・**キャンセルを含む全経路**。provider 不在の即時失敗も同期実行せずメインへディスパッチする。
2. **キャンセル時は `cancelAll()` 自身が `.cancelled` を各リクエストへ 1 回だけ返す**（0 回ではない）。`Progress.cancel()` が公式に保証するのは cancellation handler の実行と `isCancelled` の更新のみで「completion が呼ばれないこと」ではないため、その後に provider から届いた結果はリクエスト ID の gate で破棄する（利用側へは通知しない = 通知は既に 1 回済んでいる）。
...
enum ClipboardLoadError: Error {
    ...
    case cancelled               // cancelAll() による中断
}
...
final class ItemProviderLoader {
    ...
    func loadFirstImageFile(completion: @escaping (Result<URL, ClipboardLoadError>) -> Void) {
        ...
        requests[id] = Request(deliver: completion)
        ...
        let progress = provider.loadFileRepresentation(forTypeIdentifier: type) { [weak self] url, error in
            ...
            if let error {
                ...
                let dest = dir.appendingPathComponent(Self.safeFileName(url: url, provider: provider))
                do {
    ...
    nonisolated private static func safeFileName(url: URL, provider: NSItemProvider) -> String {
        let allowed: Set<String> = ["png", "jpg", "jpeg", "heic", "heif", "gif", "tiff", "webp"]
        let suggestedExt = (provider.suggestedName as NSString?)?.pathExtension ?? ""
        let ext = [url.pathExtension, suggestedExt]
            .map { $0.lowercased() }
    ...
    deinit { requests.values.forEach { $0.progress?.cancel() } }
}
```

テキストの読み取りも同じ契約に従う（ファイルを伴わないため `workDirectory` は nil）。
...
extension ItemProviderLoader {
    func loadFirstText(completion: @escaping (Result<String, ClipboardLoadError>) -> Void) {
        let id = nextId
        nextId += 1
        // Result<String, _> を Result<URL, _> の管理表へ載せるため、実装では
        // Request をジェネリック化するか型ごとに管理表を分ける（設計時に確定させる）
        registerTextRequest(id: id, completion: completion)
        ...
        attachProgress(progress, to: id)
    }
}
```

`registerTextRequest` / `finishText` / `attachProgress` は上記 `Request` と同じ「登録 → gate → 1 回配信 → キャンセル時 `.cancelled`」の規約に従う。実装設計時に `Request` をジェネリック化して 1 つの管理表へ統合するか、型ごとに分けるかを決める。

Swift 6 対応の注意: 上記は `SWIFT_VERSION = 5.0` を前提とする。`deinit` から `@MainActor` 隔離プロパティへアクセスする点も Swift 6 では診断対象になりうる。Xcode 26.3 の strict concurrency 診断では、`@Sendable` completion 内で非 Sendable な `NSItemProvider` / `NSItemProviderReading` / callback を捕捉する警告が出る。設計段階では `@MainActor` 付きの loader（上記）に加え、`withCheckedThrowingContinuation` で包んだ async API を公開形として検討する（要検証）。
