# macOS クリップボード機能 調査企画書 v2

- 作成日: 2026-08-29
- 改訂日: 2026-08-29（v2: 第 1 回レビュー指摘を全件反映）
- 前版: `artifact/features/clipboard/plans/2026-08-29-macos-clipboard-research.md`
...
| P-2 `append(_:scope:)` | `UIPasteboard.addItems(_:)` | 対応（挙動差あり） | C-03 | macOS の `writeObjects(_:)` は所有権取得なしで呼べば追記になる。ただし他アプリが所有者の場合の追記可否は未確認 → **V-13** |
...
| **RK-21** | **寿命（新設）** | **`NSFilePromiseProvider.delegate`（F-03）は `weak`。かつ `NSFilePromiseProviderDelegate` には L-03 に相当する完了通知が存在しない** | **delegate をローカル変数だけで生成すると、C-03 の直後に解放され、F-05 / F-06 が呼ばれずファイル約束が無言で失敗する。provider 自体もアプリ側が保持しないと同様** | **provider と delegate の双方を強参照するセッション型を設け、ペーストボードの所有権を保持している間は生存させる。解放判断は M-01 の `changeCount` が取得時の値と異なることで行う（所有権喪失＝約束不要）。F-06 の `completionHandler` は **1 回の書き出し要求につきちょうど 1 回**呼ぶ実装契約とする。書き出し要求は複数回来うるため、provider 全体で 1 回に絞るようなガードは置かない（2 回目以降の要求で completion が呼ばれず、受け手が待ち続ける）。システム側が provider / delegate を保持するかは V-11 で検証する** |
...
| ID | 内容 | 検証方法 |
|---|---|---|
| V-1 | Q-01〜Q-07（`types` / `availableType(from:)` / `canReadItem` / `canReadObject`）および M-01（`changeCount`）がペーストボードアクセスアラートを発生させないか | macOS 15.4 実機で `accessBehavior` を `.ask` にした状態で各 API を単独実行し、アラートおよび System Settings のペーストボード欄への登録有無を確認する。**この結果が出るまで RK-01 の Q 群フォールバックは正式採用しない** |
| V-2 | U-01〜U-04（`PasteButton`）/ U-08（`paste:` action）/ U-09（`readSelection(from:)`）が「user originated かつ paste related」としてアラート免除に該当するか | 同上の設定で各経路から貼り付けを実行し、アラート表示の有無を比較する。`.alwaysDeny` 設定下でも読み取れるかも確認する |
...
| **V-11** | **F-01 / F-03 について、システムが provider / delegate を保持するか（`weak` delegate の実効寿命）** | **`FilePromiseSession` を使わず、provider と delegate をローカル変数だけで生成して C-03 の後にスコープを抜ける。その後 Finder 等へファイルをドロップし、F-05 / F-06 が呼ばれるかを確認する。呼ばれない場合は RK-21 のセッション保持が必須であることの実証になる** |
| **V-12** | **U-05〜U-07（`copyable` / `cuttable` / `pasteDestination`）がアラート免除に該当するか、および Edit メニュー項目が自動で有効化されるか** | **`accessBehavior` を `.ask` にした状態で Edit メニューから Paste を実行し、アラート表示の有無を確認する。あわせてメニュー項目の enable / disable がペーストボード内容に応じて自動更新されるか（`PasteButton` の RK-16 と同じ制約があるか）を確認する** |
| **V-13** | **C-03 による追記（iOS の `append` 相当）が、他アプリが所有者のペーストボードに対しても成立するか** | **別アプリでコピーした直後に、所有権を取得せず `writeObjects(_:)` を呼び、既存アイテムが保持されたまま追加されるか、あるいは失敗するかを確認する** |
...
- [x] 実装リスクを権限・制約・互換性の観点で RK-01〜RK-21 として整理した
...
- [ ] File Promise を公開機能に含めるかを決め、含める場合は provider / delegate のセッション保持と解放契約を設計する（RK-21 / V-11）
- [ ] 変更監視のポーリング間隔既定値とアプリ非アクティブ時の停止方針を決める（RK-11）
- [ ] V-1〜V-13 の検証を実機で消化し、結果を設計書または本書の改訂版へ反映する。**V-8 は正の対照が成立した記録とセットで残す**
