...
> **正誤 1（2026-08-15 追記）**: 初版で T-14（DocC）を「未着手」と記載したが**誤り**。
> `ios/IosLibrary/IosLibrary/IosLibrary.docc/IosLibrary.md` に Clipboard セクションが
> 102〜181 行（約 80 行）すでに存在する。Quick Start / Threading / Privacy / 非永続性 /
> append の非保証 / cleanup / Cancellation / 検出のキャンセル不可を含む。
> 設計書のタスク表のみを参照し実ファイルを確認しなかったことが原因。
> **T-14 に残るのは新規作成ではなく内容の見直し**であり、M-08 の結論を反映する
> `### Named / unique pasteboards are not persistent`（143 行〜）の訂正が該当する。
...
| T-14 | DocC の Clipboard セクション。**内容見直しのみ**（正誤 1 を参照） |
