...
- ○ Bridge callback（`UnityIosShareManagerBridge.h` の "invoked on the main thread" 契約）が全経路（成功・失敗・JSON不正）で main actor 上に統一されている（v2 で反映、コードレビューで確認。専用のスレッド検証テストは未追加）
