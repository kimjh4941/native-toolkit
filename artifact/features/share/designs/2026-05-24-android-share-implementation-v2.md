...
**既存実装との踏襲方針:**
`UnityAndroidShareManager` は `UnityAndroidNotificationManager`（`unity_android_plugin/src/main/java/android/unity/notification/UnityAndroidNotificationManager.kt`）と同一の `executeOperation` パターン・エラーハンドリング構造を踏襲する。`executeOperation` は `IllegalArgumentException` / `ActivityNotFoundException` / `SecurityException` / `Exception` の順にキャッチし `ShareOperationListener.onShareOperation` に通知する。
...
**制御フロー:**
1. Manager が JSON をパース → `UnityShareTextSpec` 生成 → `ShareContent` に変換
2. `ShareUseCases(context).shareText(content)` 呼び出し
3. `ShareTextUseCase.invoke(content)` が `content.text.isBlank()` を検証 → `repository.shareText(content)`
4. `ShareRepositoryImpl` が `Intent.ACTION_SEND` を組み立て `createChooser` 経由で `startActivity`
   - API 34+: `UnityShareTextSpec.chooserActions` が存在すれば `EXTRA_CHOOSER_CUSTOM_ACTIONS` に追加

**URL シェアはテキストシェアと同実装。**`EXTRA_TEXT` に URL 文字列を渡し `mimeType = "text/plain"` を設定する。
...
`ShareContent` はテキスト系シェア専用。画像・ファイルシェアは `filePath(s)` を直接引数として渡す設計とする（`ShareContent` は使用しない）。

**制御フロー:**
1. Manager が画像パス文字列（1件または List）と mimeType を受け取る
2. `ShareImageUseCase.invoke(filePath, mimeType)` が mimeType の blank チェックを実施
...
画像シェアと同実装パス。MIME 型は `ShareMimeTypeHelper.getMimeType(file)` で動的判定する。単一 / 複数で `ACTION_SEND` / `ACTION_SEND_MULTIPLE` を使い分ける。
...
3. RepositoryImpl が `ShortcutManagerCompat.getMaxShortcutCountPerActivity` で上限確認
4. `Bitmap` 復元 → `ShortcutInfoCompat` 構築 → `ShortcutManagerCompat.pushDynamicShortcut`
5. 戻り値が `false` の場合 `DirectShareRegistrationFailed("quota_exceeded")` を throw

**削除:**
1. `RemoveDirectShareTargetsUseCase.invoke(ids)` が空リスト検証
2. `ShortcutManagerCompat.removeLongLivedShortcuts`
...
`ChooserAction` は UI 制御専用のプレゼンテーション概念であるため、Domain 型 `ShareContent` には持たせない。
`UnityShareTextSpec` に `chooserActions: List<UnityChooserActionSpec>` を持たせ、Manager が RepositoryImpl へ別引数として渡す。API 34 未満では無視する。

**ファイル:**
- `data/repository/ShareRepositoryImpl.kt`（`shareText` の ChooserAction 組み立て）
- `unity_android_plugin/src/main/java/android/unity/share/UnityShareSpecs.kt`（`UnityChooserActionSpec` 追加）
...
3. RepositoryImpl が BroadcastReceiver を `ContextCompat.registerReceiver(..., RECEIVER_NOT_EXPORTED)` で登録
4. `onReceive` でワンショット解除（先頭で `ctx?.unregisterReceiver(this)` を呼ぶ）
5. API バージョン分岐でパッケージ名を取得（`null` = ユーザーキャンセル）
6. `onResult(packageName)` → Manager の `ShareOperationListener.onShareResult` を呼ぶ

**`clearShareOperationListener` との関係:**
Listener を clear してもすでに登録済みの BroadcastReceiver は生存し続ける。`onReceive` でワンショット解除後に Listener が `null` であれば `onShareResult` 呼び出しをスキップする。
...
// shareJson: { "text": "...", "title": "...", "subject": "...", "mimeType": "text/plain",
...
// コールバック付きシェア（テキスト・URL のみ）
fun shareWithCallback(context: Context, shareJson: String)
// shareJson: { "text": "...", "title": "..." }

interface ShareOperationListener {
    /** シェア操作の成否通知。shareWithCallback 以外のすべての操作で呼ばれる。 */
    fun onShareOperation(operation: String, isSuccessful: Boolean, errorMessage: String?)

    /**
     * シェア選択結果の通知。shareWithCallback 操作のみで呼ばれる。
     *
     * @param operation 操作名（常に "shareWithCallback"）
     * @param selectedPackageName 選択されたアプリのパッケージ名。ユーザーキャンセル時は null
     */
...
interface ShareRepository {
    ...
    /**
     * テキストをシェアし、ユーザーが選択したアプリのパッケージ名をコールバックで返す。
     *
     * onResult はシェア完了後に選択されたパッケージ名（キャンセル時は null）で
     * 一度だけ呼ばれることを保証する。
     *
     * @param content シェアするテキスト内容
     * @param onResult 選択パッケージ名（キャンセル時は null）を受け取るコールバック
     */
...
| `DirectShareRegistrationFailed("quota_exceeded")` | `pushDynamicShortcut` 上限超過 | `"Failed to register Direct Share target: quota exceeded."` |
| `DirectShareRegistrationFailed(reason)` | その他登録失敗 | `"Failed to register Direct Share target: ${reason}"` |
...
### `ShareContent`（Domain — テキスト・URL シェア専用）

`mimeType` の blank チェックは `ShareTextUseCase` で実施する。
...
### `UnityShareTextSpec`（Bridge 入力モデル）

`chooserActions` は Bridge 層のみで保持し Domain に持ち込まない。
...
### `UnityChooserActionSpec`（Bridge 入力モデル — API 34+ のみ使用）

```kotlin
data class UnityChooserActionSpec(
    val label: String,
    val iconBase64: String,
    val intentAction: String = Intent.ACTION_SEND
)
// JSON スキーマ: { "label": "...", "iconBase64": "...", "intentAction": "..." }
```

### `UnityShareImageSpec`（Bridge 入力モデル — 単一画像用）

```kotlin
data class UnityShareImageSpec(
    val filePath: String,
    val mimeType: String = "image/*"
)
// JSON スキーマ: { "filePath": "...", "mimeType": "image/jpeg" }
```

### `UnityShareFileSpec`（Bridge 入力モデル — 複数画像 / ファイル共用）

```kotlin
data class UnityShareFileSpec(
    val filePath: String? = null,       // 単一ファイルシェア用
    val filePaths: List<String> = emptyList(),  // 複数ファイルシェア用
    val mimeType: String? = null
)
// JSON スキーマ: { "filePath": "...", "mimeType": "..." }
//            or { "filePaths": ["...", "..."] }
```
...
// JSON スキーマ: { "id": "...", "label": "...", "iconBase64": "...", "category": "..." }
```
...
| mimeType 空文字 | `ShareTextUseCase` | `ShareDomainError.InvalidMimeType` が throw される |
| 単一画像シェア正常系 | `ShareImageUseCase` | `repository.shareImage` が1回呼ばれる |
| 単一画像 mimeType 空文字 | `ShareImageUseCase` | `ShareDomainError.InvalidMimeType` が throw される |
...
| ファイルシェア FileNotFound | `ShareFileUseCase` | `ShareDomainError.FileNotFound` が throw される |
| ファイルシェア IllegalFileAccess | `ShareFileUseCase` | `ShareDomainError.IllegalFileAccess` が throw される |
| 複数ファイルシェア空リスト | `ShareMultipleFilesUseCase` | `ShareDomainError.EmptyFileList` |
...
| Direct Share 登録: InvalidBase64Icon | `RegisterDirectShareTargetUseCase` | `ShareDomainError.InvalidBase64Icon` が throw される |
...
| コールバック キャンセル | `ShareWithCallbackUseCase` | `onResult(null)` が呼ばれる |
...
| chooserActions 配列のパース | `UnityChooserActionSpec` の List が正しく返る |
| JSON 不正形式 | JSONException が throw される |

**対象: `UnityAndroidShareManager`（executeOperation エラー変換）**

| テストケース | 確認内容 |
|---|---|
| `ShareDomainError.FileNotFound` | `onShareOperation(isSuccessful=false, errorMessage="File not found: ...")` が呼ばれる |
| `ShareDomainError.EmptyContent` | `onShareOperation(isSuccessful=false, errorMessage="Share content is empty...")` が呼ばれる |
| `ActivityNotFoundException` | `onShareOperation(isSuccessful=false, errorMessage="No app available...")` が呼ばれる |
| 正常系 | `onShareOperation(isSuccessful=true, errorMessage=null)` が呼ばれる |
| `onShareResult` 呼び出し | `shareWithCallback` 完了後 `onShareResult(OPERATION_SHARE_WITH_CALLBACK, packageName)` が呼ばれる |
| キャンセル時 | `onShareResult(OPERATION_SHARE_WITH_CALLBACK, null)` が呼ばれる |
...
| コールバック キャンセル | `onShareResult(null)` が呼ばれる | API 31, 34 |
...
**完了条件:** Domain 型のみ、KDoc 付き（`shareWithCallback` にワンショット保証の記載）、コンパイル通過
...
**完了条件:** 各 UseCase が `operator fun invoke` を持ち、MockShareRepository で単体テスト（正常系・全エラー系）通過
...
サンプルアプリ（consumer app）側で設定する。本タスクは設定内容の定義のみを扱い、ファイル作成はサンプルアプリのタスクで実施する。

**`file_paths.xml` に定義するパス種別:**

```xml
<?xml version="1.0" encoding="utf-8"?>
<paths>
    <files-path name="files" path="." />           <!-- context.filesDir -->
    <cache-path name="cache" path="." />            <!-- context.cacheDir -->
    <external-files-path name="external_files" path="." />  <!-- context.getExternalFilesDir(null) -->
</paths>
```

**`proguard-rules.pro` に追加する keep ルール:**
...
`ShareOperationListener` に結果を通知する。単体テスト（JSON パーサー・Manager エラー変換）が通過する
...
- `unity_android_plugin/src/test/.../share/UnityAndroidShareManagerTest.kt`（新規）
...
サンプルアプリの実装は design-sample-app で行う。本タスクは完了条件と依存関係のみ定義する。
...
| `RECEIVER_NOT_EXPORTED` 未設定 | Android OS がクラッシュを引き起こす | `ContextCompat.registerReceiver` に `RECEIVER_NOT_EXPORTED` を指定し API バージョン分岐不要にする |
...
| Direct Share 登録上限超過 | `pushDynamicShortcut` が `false` を返す | `getMaxShortcutCountPerActivity` で事前チェックし、上限超過時は `DirectShareRegistrationFailed("quota_exceeded")` を throw |
...
| Predictive Back（API 33+） | Chooser UI の戻る動作が変化 | Chooser 側の制御のため `enableOnBackInvokedCallback` の設定は不要 |
