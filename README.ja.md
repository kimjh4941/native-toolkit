...
- iOS: `UIAlertController` + 通知 + シェア + クリップボードベースのネイティブ API
- macOS: `NSAlert` / `NSOpenPanel` / `NSSavePanel` + 通知 + シェア + クリップボードベースのネイティブ API
...
- マニュアル: `docs/1.10.0/manual/index.ja.md`
- 公開ドキュメント: `docs/1.10.0/manual/`
...
- `ios/IosLibrary`
  - 中核: `IosDialogManager` / `IosNotificationManager` / `IosShareManager` / `IosClipboardManager`
  - 対応: Alert / Confirm / Destructive / ActionSheet / TextInput / Login / Notification / Share / Clipboard
  - Doc: DocC
...
- `mac/MacLibrary`
  - 中核: `MacDialogManager` / `MacNotificationManager` / `MacShareManager` / `MacClipboardManager`
  - 対応: Alert / File / MultiFile / Folder / MultiFolder / Save / Notification / Share / Clipboard
  - Doc: DocC
