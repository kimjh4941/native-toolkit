...
- 매뉴얼: `docs/1.10.0/manual/index.ko.md`
- 배포 문서: `docs/1.10.0/manual/`
...
- iOS:
  - `dist/1.10.0/ios/ios-native-toolkit-1.3.0.xcframework`
  - `dist/1.10.0/ios/unity-ios-native-toolkit-1.3.0.xcframework`
- macOS:
  - `dist/1.10.0/mac/mac-native-toolkit-1.3.0.xcframework`
  - `dist/1.10.0/mac/unity-mac-native-toolkit-1.3.0.xcframework`
...
- `android/android_library`
  - 핵심: `AndroidDialogFragment` / Android 알림 API / `ShareUseCases` / `ClipboardUseCases`
...
- `ios/IosLibrary`
  - 핵심: `IosDialogManager` / `IosNotificationManager` / `IosShareManager` / `IosClipboardManager`
  - 지원: Alert / Confirm / Destructive / ActionSheet / TextInput / Login / Notification / Share / Clipboard
  - 문서: DocC (`.docc`)
...
- `mac/MacLibrary`
  - 핵심: `MacDialogManager` / `MacNotificationManager` / `MacShareManager` / `MacClipboardManager`
  - 지원: Alert / File / MultiFile / Folder / MultiFolder / Save / Notification / Share / Clipboard
  - 문서: DocC (`.docc`)
...
- `windows/WindowsLibrary`
  - C 형태 API (예: `showAlertDialog`, `showFileDialog`, `showFolderDialog`, `showNotification`, `scheduleNotification`)
  - 헤더: `windows/WindowsLibrary/WindowsDialogManager.h`, `windows/WindowsLibrary/WindowsNotificationManager.h`
...
```bash
./scripts/publish_docs.sh 1.10.0
```

생성 없이 복사만 할 경우:

```bash
./scripts/publish_docs.sh 1.10.0 --skip-build
```
