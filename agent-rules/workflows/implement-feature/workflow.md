...
2. インタラクティブ入力でパラメータを確定する（必須）
   ...
   - 入力がない場合は `artifact/features/<feature>/designs/` 配下の `*-design*.md` を探索し、同一ドキュメントの改訂版（`-v2`, `-v3` など）がある場合は最も大きいバージョンのみを候補として提示する
...
4. 実装制約を固定する（必須）
   ...
   - `common.md`「システム API に合わせた同期・非同期設計」と設計書の対応表を固定し、Repository / UseCase / private helper を不必要に非同期化しない
   - Manager の公開 API 規約による薄い async wrapper と、実際のシステム API / 下位層の実行方式を区別する
...
6. 実装を行う（必須）
   ...
   - 設計書の同期・非同期レイヤー対応表どおりにシグネチャと actor isolation を実装する
   - システム API と異なる方式へ変換する箇所は薄い adapter に限定し、変換理由をコードコメントまたは実装結果へ記録する
...
7. テストを実装・更新する（必須）
   ...
   - 同期 API は同期完了と不要な callback / task 化がないことを確認する
   - 非同期 API は完了 thread / actor、exactly-once、キャンセル、キャンセルと完了の競合、リソース解放を検証する
...
9. 実装結果を検証する（必須）
   ...
   - 同期・非同期レイヤー対応表と実装シグネチャ / actor isolation / キャンセル契約を照合し、各操作の一致状況を報告する
...
- 同期・非同期レイヤー対応表との一致状況を必ず報告し、差分がある場合は理由と影響を明記する
...
検査を追加したら壊して確認する。そのとき、**前回のレビュー指摘をそのまま戻す変異だけを作らない**。
それでは指摘された形しか塞げない。

| 作る変異 | 例 |
|---|---|
| **間接参照を 1 つ挟む** | 値を返す computed property / ヘルパーを足し、それ経由で読む |
| **名前を変える** | 検査がリテラルに依存していないか |
| **順序を入れ替える** | 2 つの値を別々に読む検査が、対にならない組を受理しないか |
| **主題を空にする** | 対象ファイルの中身を全部消しても検査が通らないか（空回り防止） |

**契約は保ったまま、実装の形だけを変えること。** 契約を壊す変異は「落ちて当然」であり、
検査の強さを測れない。
